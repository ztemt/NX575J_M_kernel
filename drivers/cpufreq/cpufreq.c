...
//NUBIA add begin for PR00372865
//Merge MTK Patch for sys file: /sys/devices/system/cpu/cpu0/stas/time_in_state
#define NO_RESET 1
//NUBIA add end
...
static int __cpufreq_add_dev(struct device *dev, struct subsys_interface *sif)
{
	...
	if (unlikely(policy)) {
		...
		pr_err("CPUFREQ policy already exists, cpu = %u\n", cpu);
		cpulist_scnprintf(str1, sizeof(str1), (const struct cpumask *)policy->cpus);
		cpulist_scnprintf(str2, sizeof(str2), (const struct cpumask *)policy->related_cpus);
		pr_err("cpus = %s, related_cpus = %s\n", str1, str2);
		pr_err("policy->cpu = %d, freq = (%d, %d, %d)\n", policy->cpu,
