...
static int general_battery_get_batt_temperature(struct general_battery *chip)
{
    ...
    int adc_volt = 5;
    int compensate_volt = 0;
    adc_volt = PMIC_IMM_GetOneChannelValue(PMIC_AUX_BATON_AP, adc_volt, 1);

#ifdef CONFIG_OF    
    if((chip->batt_ma < 0)&&(chip->usb_present==1))
    {
    	compensate_volt = abs(chip->batt_ma)/100;
    	adc_volt = adc_volt - compensate_volt;
      BAT_DEG(BAT_LOG_CRTI,"BATT:CHG adc_volt=%d, compensate_volt=%d \n",adc_volt,compensate_volt);
    }
#endif
...
#ifdef CONFIG_OF
    //by Colin for batt temperature calibration
    if(chip->batt_ma < 0){
    	BAT_DEG(BAT_LOG_CRTI,"raw batt temperature = %d \n",chip->batt_temp);
    	chip->batt_temp = chip->batt_temp + 10*(abs(chip->batt_ma)/600);
    }
#endif
