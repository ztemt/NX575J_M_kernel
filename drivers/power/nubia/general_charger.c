...
struct general_charger
{
    ...
    unsigned int		usb_max_chg_current;
    ...
    int                 battery_max_charge_current;
...
#ifdef CONFIG_OF
static int battery_common_fb_notifier_cb(struct notifier_block *self,
		unsigned long event, void *data)
{
	int *transition;
	struct fb_event *evdata = data;
	pr_err("battery_common_fb_notifier_cb.\n");
	if ( evdata && evdata->data ) {
		if ( event == FB_EVENT_BLANK ) {
			transition = evdata->data;
			if( *transition == FB_BLANK_POWERDOWN )
			{
				g_temperature_delta = 0;
			}
			else if ( *transition == FB_BLANK_UNBLANK )
			{
				g_temperature_delta = 20;
			}
		}
	}
	return 0;
...
static void general_charger_get_charger_detection(struct general_charger *info)
{
	...
	if(info->usb_chgr_type ^ usb_chgr_type) {
		if( usb_supply_type == POWER_SUPPLY_TYPE_USB ) {
            ...
            info->usb_max_chg_current = info->usb_hvdcp_limit_max_current;
        }
        else {
...
#define PARALLEL_CHARGER_LIMIT_CURRENT 1500
static int general_charger_set_usb_charge_current(struct general_charger *info, int current_ma)
{
    ...
    if(usb_current==0){
     	  info->parallel_chgr(CHARGER_COMMON_CMD_ENABLE, &disable);
        info->main_chgr(CHARGER_COMMON_CMD_ENABLE, &disable);
        return 0;
    }
    
    if(usb_current > PARALLEL_CHARGER_LIMIT_CURRENT){
    	  info->parallel_enabled = true;
        ...
        info->parallel_chgr(CHARGER_COMMON_CMD_ENABLE, &enable);
        info->main_chgr(CHARGER_COMMON_CMD_ENABLE, &enable);
        
        BAT_DEG(BAT_LOG_CRTI, "BATT:CHG general_charger_set_usb_charge_current--Parallel charge current:%d:%d", usb_current, parallel_current);  
    }
    else
    {
...
{
	int ret = 0;
	unsigned int chg_current = info->battery_max_charge_current;  //battery max current
	unsigned int usb_max_chg_current = info->usb_max_chg_current; //usb max current
	chg_current = min(chg_current, info->thermal_current);

	pr_debug("setting usb limit current %d mA\n", chg_current);
	info->usb_chg_current = chg_current;
		
  BAT_DEG(BAT_LOG_CRTI, "BATT:CHG  set charge current   usb_max_chg_current=%d,battery_max_charge_current=%d,thermal_current=%d \n", 
		                     usb_max_chg_current,info->battery_max_charge_current, info->thermal_current);
				
	general_charger_set_usb_limit_current(info, usb_max_chg_current); 
	general_charger_set_usb_charge_current(info, chg_current);		
...
static void general_charger_jeita_setting_checkpoint(struct general_charger *info)
{
    ...
    state_index = general_charger_get_battery_status(batt_temp_map,
                  ...
                  (info->batt_temp + g_temperature_delta),
                  &battery_status);
#else
    state_index = general_charger_get_battery_status(batt_temp_map,
                  ARRAY_SIZE(batt_temp_map),
    ...
    BAT_DEG(BAT_LOG_CRTI,"BATT:CHG jeita  batt_temp=%d batt_status=%d batt_current=%d \n",info->batt_temp,battery_status,batt_current);
    ...
    if(battery_status != info->batt_health){
       ...
       BAT_DEG(BAT_LOG_CRTI,"BATT:CHG batt_temp=%d batt_status=%d batt_current=%d start charging...\n",info->batt_temp,battery_status,batt_current);
...
/*static void general_charger_try_enable_parallel_charge(struct general_charger *info)
{
    ...
    general_charger_set_usb_charge_current(info, info->battery_max_charge_current);
...
static void general_charger_chgr_monitor_work(struct work_struct *work)
{
    ...
    if((info->usb_present != 1) && wake_lock_active(&info->monitor_wake_lock)) {
        ...
        //disable parallel charger 20170118
#ifdef CONFIG_NUBIA_PARALLEL_CHARGER_SUPPORT
        info->parallel_chgr(CHARGER_COMMON_CMD_ENABLE, &disable);
#endif      
    }
...
static int general_charger_parse_dt_parameters(struct general_charger *info)
{
    ...
    rc = of_property_read_u32(node, "nubia,battery_max_charge_current",
                                               &info->battery_max_charge_current);
    if (rc < 0)
        info->battery_max_charge_current = -EINVAL;
...
static int general_charger_probe(struct platform_device *pdev)
{
	...
	fb_notifier.notifier_call = battery_common_fb_notifier_cb;
...
#ifdef CONFIG_NUBIA_PARALLEL_CHARGER_SUPPORT
extern void bq25896_batfet_disable(void);
static void general_charger_shutdown(struct platform_device *dev)
{
	  bq25896_batfet_disable();
}
#endif
...
#ifdef CONFIG_NUBIA_PARALLEL_CHARGER_SUPPORT
    .shutdown = general_charger_shutdown,
#endif
    .driver = {
        .name = "general_charger",
    #ifdef CONFIG_PM_SLEEP
