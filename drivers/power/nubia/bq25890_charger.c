...
static void bq25890_dump_register(void);
static void bq25890_chg_en(unsigned int val);
static void bq25890_set_vreg(unsigned int val);
static unsigned int bq25890_get_ichg(void);
static unsigned int bq25890_get_vbus_state(void);
...
static void bq25890_dump_register(void);
static unsigned int bq25890_get_vbus(void);

static unsigned int charging_error;
static unsigned int bq25890_charger_get_error_state(void);
static void bq25890_set_ichg(unsigned int val);
static int bq25890_get_otg_status(void);
static void bq25890_set_otg_status(unsigned int val);
static unsigned int bq25890_get_charger_online(void);
static unsigned int bq25890_get_vbat(void);
/* ============================================================ // */
...
unsigned char SHIFT)
{
    ...
    BAT_DEG(BAT_LOG_FULL, "[bq25890_read_interface] Reg[%x]=0x%x\n", RegNum, bq25890_reg);

    bq25890_reg &= (MASK << SHIFT);
    *val = (bq25890_reg >> SHIFT);

    BAT_DEG(BAT_LOG_FULL, "[bq25890_read_interface] val=0x%x\n", *val);

    return ret;
}

unsigned int bq25890_config_interface(unsigned char RegNum, unsigned char val, unsigned char MASK, unsigned char SHIFT)
{
    unsigned char bq25890_reg = 0;
    unsigned int ret = 0;
    ...
    BAT_DEG(BAT_LOG_FULL, "[bq25890_config_interface] write Reg[%x]=0x%x\n", RegNum,
    bq25890_reg);
...
static unsigned int bq25890_get_iinlim(void)
{
    unsigned int ret = 0;
    unsigned char val = 0;
...
static unsigned int bq25890_get_chg_en(void)
{
    unsigned int ret = 0;
    unsigned char val = 0;
...
static void bq25890_set_sys_min(unsigned int val)
{
    unsigned int ret = 0;
...
static void bq25890_set_vrechg(unsigned int val)
{
    unsigned int ret = 0;
...
/* CON7---------------------------------------------------- */
static void bq25890_en_term_chg(unsigned int val)
{
    unsigned int ret = 0;
...
static void bq25890_set_chg_timer(unsigned int val)
{
    unsigned int ret = 0;
...
/* CON8--------------------------------------------------- */
static void bq25890_set_thermal_regulation(unsigned int val)
{
    unsigned int ret = 0;
...
static void bq25890_set_boost_vlim(unsigned int val)
{
    unsigned int ret = 0;
...
/* CON0C---------------------------------------------------- */
static unsigned int bq25890_get_chrg_fault_state(void)
{
    unsigned int ret = 0;
    unsigned char val = 0;
...
#ifdef BQ25890_REGISTER_STATE_DUMP
...
static unsigned int bq25890_get_boost_state(void)
{
    unsigned int ret = 0;
    unsigned char val = 0;
...
static unsigned int bq25890_get_bat_state(void)
{
    unsigned int ret = 0;
    unsigned char val = 0;
...
static unsigned int bq25890_get_charger_online()
{
	unsigned int ret = 0;
	unsigned char val;
...
/* CON13 */
#ifdef BQ25890_REGISTER_STATE_DUMP
static unsigned int bq25890_get_idpm_state(void)
{
    unsigned int ret = 0;
    unsigned char val = 0;
...
static void bq25890_dump_register(void)
{
    ...
    for (i = 0; i < BQ25890_REG_NUM; i++)
    {
        ...
        BAT_DEG(BAT_LOG_CRTI, "[bq25890 reg@][0x%x]=0x%x ", i, bq25890_reg[i]);
    }
    bq25890_ADC_start(1);
    ...
    "[PE+]BQ25890 Ichg_reg=%d mA, Iinlin=%d mA, Vbus=%d mV, err=%d",
    ichg_reg * 64, iinlim * 50 + 100, vbus * 100 + 2600, fault);
    BAT_DEG(BAT_LOG_CRTI, "[PE+]BQ25890 Ichg=%d mA, Vbat =%d mV, ChrStat=%d, CHGEN=%d, VDPM=%d\n",
...
static unsigned int bq25890_charger_hw_init(void *data)
{
    unsigned int status = STATUS_OK;
    bq25890_set_en_ilim(0x00);      //enable ilimit pin
    bq25890_config_interface(BQ25890_CON0, 0x0, 0x03, 6);	/* Disable ilimit Pin */
    bq25890_config_interface(BQ25890_CON0, 0x1C, 0x3F, 0);	/* enable ilimit Pin */
	#if defined(CONFIG_NUBIA_PM_BQXXX_VINDPM_400MV)
		bq25890_config_interface(BQ25890_CON1, 0x0, 0x01, 0); /*Vindpm offset 0x0 400mv */
	#else
		bq25890_config_interface(BQ25890_CON1, 0x1, 0x01, 0);	/* Vindpm offset  0x1 600MV */
	#endif
    bq25890_config_interface(BQ25890_CON1, 0x0, 0x01, 1);	/* Disable 12V HVDCP */

    bq25890_config_interface(BQ25890_CON2, 0x0, 0x1, 5);	/* boost freq 1.5MHz when OTG_CONFIG=1 */
    bq25890_set_ico_en_start(0x01); //enable ico Algorithm
    bq25890_config_interface(BQ25890_CON2, 0x1, 0x1, 4);	/* enable ico Algorithm -->bear:en */
    bq25890_config_interface(BQ25890_CON2, 0x1, 0x1, 3);	/* enable HV DCP  */
    bq25890_config_interface(BQ25890_CON2, 0x1, 0x1, 2);	/* enable MaxCharge  */
    bq25890_config_interface(BQ25890_CON2, 0x1, 0x1, 1);	/* disable DPDM detection */
    bq25890_config_interface(BQ25890_CON2, 0x1, 0x1, 0);	/* enable auto DPDM detection */

    bq25890_config_interface(BQ25890_CON3, 0x0, 0x1, 6);
	bq25890_config_interface(BQ25890_CON3, 0x1, 0x1, 4);	/* System min voltage default 3.5V */
    bq25890_set_sys_min(0x03);   //System min voltage default 3.5V
    bq25890_config_interface(BQ25890_CON3, 0x3, 0x7, 1);	/* System min voltage default 3.5V */

    /*PreCC mode */
    bq25890_set_iprechg(0x07);  //precharge current default 448mA
    bq25890_set_iterml(0x01);   //termianation current default 128mA
    bq25890_config_interface(BQ25890_CON5, 0x7, 0xF, 4);	/* precharge current default 448mA */
    bq25890_config_interface(BQ25890_CON5, 0x1, 0x7, 0);	/* termianation current default 128mA */
    bq25890_set_batlowv(0x01); //precharge2cc voltage,BATLOWV, 3.0V 
    bq25890_config_interface(BQ25890_CON6, 0x1, 0x1, 1);	/* precharge2cc voltage,BATLOWV, 3.0V */ 
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
	pr_err("4.40V support\n");
...
#endif
    bq25890_set_vrechg(0x00);
    bq25890_config_interface(BQ25890_CON6, 0x0, 0x1, 0);	/* recharge voltage@VRECHG=CV-100MV */
    bq25890_en_term_chg(0x01); // enable ICHG termination detect
    bq25890_config_interface(BQ25890_CON7, 0x1, 0x1, 7);	/* enable ICHG termination detect */
    bq25890_set_wd_timer(0x00); //disable watchdog sec
    bq25890_set_chg_timer(0x00); //set charging timer 5h
    bq25890_en_chg_timer(0x01); //enable charging timer
    bq25890_config_interface(BQ25890_CON7, 0x00, 0x3, 4);	/* disable  watch dog  secs 0x3 */
    bq25890_config_interface(BQ25890_CON7, 0x1, 0x1, 3);	/* enable charging timer safty timer */
    bq25890_config_interface(BQ25890_CON7, 0x0, 0x3, 1);	/* charging timer 5h */
    bq25890_config_interface(BQ25890_CON7, 0x1, 0x1, 0);	/* JEITA_ISet : 20% x ICHG */

    bq25890_set_vbat_clamp(0x00);
    bq25890_set_vbat_ir_compensation(0x00);
    bq25890_set_thermal_regulation(0x03);   //thermal 120 as default
    bq25890_config_interface(BQ25890_CON8, 0x3, 0x7, 5);	/* enable ir_comp_resistance */
    ...
    bq25890_set_boost_ilim(0x01);   //boost current limit 0.5A
    bq25890_set_boost_vlim(0x04);   //boost voltagte 4.998V default
    bq25890_config_interface(BQ25890_CONA, 0x7, 0xF, 4);	/* boost voltagte 4.998V default */
    bq25890_config_interface(BQ25890_CONA, 0x3, 0x7, 0);	/* boost current limit 0.5A */

    /*Vbus current limit */
    bq25890_set_force_vindpm(0x00);     //vindpm vth 0:relative 1:absolute
    //bq25890_set_vindpm(0x15);
    bq25890_config_interface(BQ25890_COND, 0x0, 0x1, 7);	/* vindpm vth 0:relative 1:absolute */
    //bq25890_config_interface(BQ25890_COND, 0x15, 0x7F, 0);	/* absolute VINDPM = 2.6 + code x 0.1 =4.6V;K2 24261 4.452V */
    return status;
}


static unsigned int bq25890_charger_dump_register(void *data)
{
    unsigned int status = STATUS_OK;
...
static unsigned int bq25890_charger_enable(void *data)
{
    unsigned int status = STATUS_OK;
    ...
    else
    {
        ...
        bq25890_chg_en(enable);
        if (bq25890_charger_get_error_state())
...
static unsigned int bq25890_charger_set_hiz_mode(void *data)
{
    unsigned int status = STATUS_OK;
    unsigned int enable = *(unsigned int *) (data);
...
static unsigned int bq25890_charger_set_cv_voltage(void *data)
{
    unsigned int status;
    ...
    array_size = GETARRAYNUM(VREG);
    status = STATUS_OK;
    /*pre_register_value = -1; */
    BAT_DEG(BAT_LOG_CRTI, "charging_set_cv_voltage set_cv_voltage=%d\n",
    *(unsigned int *) data);
    set_cv_voltage = bmt_find_closest_level(VREG, array_size, *(unsigned int *) data);
    //battery_set_cv_voltage(set_cv_voltage);
    register_value =
    charging_parameter_to_value(VREG, GETARRAYNUM(VREG), set_cv_voltage);
...
static unsigned int bq25890_charger_get_charging_current(void *data)
{
    unsigned int ibatt;
    ...
    /*Get current level */
    array_size = GETARRAYNUM(ICHG);
    val = bq25890_get_ichg();

    ibatt = charging_value_to_parameter(ICHG, array_size, val);
    return ibatt;
}

static unsigned int bq25890_charger_get_charger_voltage(void *data)
{
	unsigned char reg;
	reg = bq25890_get_vbus();
	return reg * 100 + 2600;
}


static unsigned int bq25890_charger_set_charging_current(void *data)
{
    unsigned int status = STATUS_OK;
    ...
    array_size = GETARRAYNUM(ICHG);
    set_chr_current = bmt_find_closest_level(ICHG, array_size, current_value);
    register_value = charging_parameter_to_value(ICHG, array_size, set_chr_current);
    bq25890_config_interface(BQ25890_CON4, register_value, 0x80, 1);
...
static unsigned int bq25890_charger_set_input_current(void *data)
{
    unsigned int status = STATUS_OK;
    unsigned int current_value = *(unsigned int *) data * 1000;
    unsigned int set_chr_current;
    ...
    array_size = GETARRAYNUM(IINLIM);
    set_chr_current = bmt_find_closest_level(IINLIM, array_size, current_value);
    register_value = charging_parameter_to_value(IINLIM, array_size, set_chr_current);
    bq25890_set_iinlim(register_value);

    return status;
}

static unsigned int bq25890_charger_get_input_current(void *data)
{
    unsigned int status = STATUS_OK;
...
static unsigned int bq25890_charger_get_charger_present(void *data)
{
	return bq25890_get_charger_online();
}

static unsigned int bq25890_charger_get_charging_status(void *data)
{
    unsigned char reg;

    bq25890_read_interface(BQ25890_CONB, &reg, 0x3, 3);	/* ICHG to BAT */
...
static unsigned int bq25890_charger_get_battery_voltage(void *data)
{
	unsigned int vbat;
	vbat = bq25890_get_vbat();
    return vbat * 20 + 2304;
...
static unsigned int bq25890_charger_get_charger_type(void *data)
{
    unsigned int type;
	type = bq25890_get_vbus_state();
    return type;
}
...
static unsigned int bq25890_charger_set_otg_status(void *data)
{
	unsigned int enable = *(unsigned int *) (data);
    if (1 == enable)
    {
        bq25890_set_en_hiz(0x0);
        bq25890_set_otg_status(enable);
    }
    else
    {
        bq25890_set_otg_status(enable);
...
static unsigned int bq25890_charger_get_otg_status(void *data)
{
	int status = bq25890_get_otg_status();
	return status;
}

static unsigned int(*charging_func[CHARGER_COMMON_CMD_NUMBER]) (void *data);

int bq25890_control_interface(CHARGER_COMMON_CTRL_CMD cmd, void *data)
{
    int status;
    ...
    {
        init = 0;
        charging_func[CHARGER_COMMON_CMD_INIT] = bq25890_charger_hw_init;
        charging_func[CHARGER_COMMON_CMD_DUMP_REGISTER] = bq25890_charger_dump_register;
        charging_func[CHARGER_COMMON_CMD_ENABLE] = bq25890_charger_enable;
        charging_func[CHARGER_COMMON_CMD_SET_CV_VOLTAGE] = bq25890_charger_set_cv_voltage;
        charging_func[CHARGER_COMMON_CMD_GET_CHARGING_CURRENT] = bq25890_charger_get_charging_current;
        charging_func[CHARGER_COMMON_CMD_SET_CHARGING_CURRENT] = bq25890_charger_set_charging_current;
        charging_func[CHARGER_COMMON_CMD_SET_INPUT_CURRENT] = bq25890_charger_set_input_current;
        charging_func[CHARGER_COMMON_CMD_GET_INPUT_CURRENT] = bq25890_charger_get_input_current;
        charging_func[CHARGER_COMMON_CMD_GET_BATTERY_VOLTAGE] = bq25890_charger_get_battery_voltage;
		charging_func[CHARGER_COMMON_CMD_GET_CHARGER_PRESENT] = bq25890_charger_get_charger_present;
        charging_func[CHARGER_COMMON_CMD_GET_CHARGING_STATUS] = bq25890_charger_get_charging_status;
        charging_func[CHARGER_COMMON_CMD_GET_CHARGER_TYPE] = bq25890_charger_get_charger_type;
		charging_func[CHARGER_COMMON_CMD_SET_OTG_STATUS] = bq25890_charger_set_otg_status;
		charging_func[CHARGER_COMMON_CMD_GET_OTG_STATUS] = bq25890_charger_get_otg_status;
		charging_func[CHARGER_COMMON_CMD_GET_CHARGER_VOLTAGE] = bq25890_charger_get_charger_voltage;
		charging_func[CHARGER_COMMON_CMD_SET_HIZ_SWTCHR] = bq25890_charger_set_hiz_mode;
    }
...
enum power_supply_property bq25890_batt_props[] = {
	POWER_SUPPLY_PROP_CONTROL_FUNCTION,
};

static int bq25890_changer_get_property(struct power_supply *psy,
			    enum power_supply_property psp,
			    union power_supply_propval *val)
{
	switch (psp) {
    case POWER_SUPPLY_PROP_CONTROL_FUNCTION:
		val->int64val = (int64_t)bq25890_control_interface;
		break;
	default:
		return -EINVAL;
	}
...
{
    BAT_DEG(BAT_LOG_CRTI, "[bq25890_driver_probe]\n");
	...
	chgr_psy.properties = bq25890_batt_props;
	chgr_psy.num_properties = ARRAY_SIZE(bq25890_batt_props);
	chgr_psy.get_property = bq25890_changer_get_property;
...
{
    BAT_DEG(BAT_LOG_CRTI, "[show_bq25890_access] 0x%x\n", g_reg_value_bq25890);
...
const char *buf, size_t size)
{
    ...
    BAT_DEG(BAT_LOG_CRTI, "[store_bq25890_access]\n");

    if (buf != NULL && size != 0)
    {
        BAT_DEG(BAT_LOG_CRTI, "[store_bq25890_access] buf is %s and size is %zu\n", buf,
        size);
...
static int bq25890_user_space_probe(struct platform_device *dev)
{
    ...
    BAT_DEG(BAT_LOG_CRTI, "******** bq25890_user_space_probe!! ********\n");
...
static int __init bq25890_init(void)
{
...
#ifdef CONFIG_OF
    BAT_DEG(BAT_LOG_CRTI, "[bq25890_init] init start with i2c DTS");
#else
    BAT_DEG(BAT_LOG_CRTI, "[bq25890_init] init start. ch=%d\n", bq25890_BUSNUM);
    ...
    if (ret)
    {
        BAT_DEG(BAT_LOG_CRTI, "****[bq25890_init] Unable to device register(%d)\n",
                    ret);
        return ret;
    }
    ...
    if (ret)
    {
        BAT_DEG(BAT_LOG_CRTI, "****[bq25890_init] Unable to register driver (%d)\n",
                    ret);
        return ret;
    }
