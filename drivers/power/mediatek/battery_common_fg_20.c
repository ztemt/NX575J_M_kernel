...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
static int full_typical_capacity;
static int full_design_capacity_umah;
#endif
/* ////////////////////////////////////////////////////////////////////////////// */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CHARGE_FULL:
        val->intval = full_typical_capacity;
        break;
    case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
        val->intval = full_design_capacity_umah;
        break;
#endif
	default:
		ret = -EINVAL;
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
	bat_data->current_now = battery_meter_get_charging_current();
#endif
	/* Dual battery */
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
	bat_data->BAT_HEALTH = BMT_status.batt_health;
#else
...
static void usb_update(struct usb_data *usb_data)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
        ...
        if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||
			...
			usb_psy->type = POWER_SUPPLY_TYPE_MAINS;
        }
...
{
	mutex_lock(&charger_type_mutex);
	hw_charging_enable_dp_voltage(ison);
	mutex_unlock(&charger_type_mutex);
}
...
{
	battery_log(BAT_LOG_CRTI, "mt_battery_charger_detect_check\n");
...
#endif
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
		battery_log(BAT_LOG_CRTI, "Force enable DPDM detection.\n");
...
void do_chrdet_int_task(void)
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT, &plug_out_aicr);
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
/*static irqreturn_t battery_common_usbin_interrupt_handler(int irq, void *data)
{
	CHARGER_TYPE CHR_Type_num = CHARGER_UNKNOWN;
	int input_current = CHARGE_CURRENT_1000_00_MA;
	int current_ma = batt_cust_data.ac_charger_current;
	int usb_present = pmic_get_register_value(PMIC_RGS_CHRDET);
	battery_log(BAT_LOG_CRTI, "#### battery_common_usbin_interrupt_handler ####\n");
	if(usb_present){
		battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
		BMT_status.charger_type = CHR_Type_num;
		if(BMT_status.charger_type == STANDARD_CHARGER){
			input_current = batt_cust_data.ac_charger_input_current;
			battery_log(BAT_LOG_CRTI,
					"[HVDCP] Default CC mode charging : %d, input current = %d\r\n",
					current_ma, input_current);
			battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
				 &input_current);
			battery_charging_control(CHARGING_CMD_SET_CURRENT, &current_ma);
		}
		else if(BMT_status.charger_type == CHARGING_HOST) {
			input_current = batt_cust_data.charging_host_charger_current;
			battery_log(BAT_LOG_CRTI,
					"[CDP] Default CC mode charging : %d, input current = %d\r\n",
					current_ma, input_current);
			battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
				 &input_current);
			battery_charging_control(CHARGING_CMD_SET_CURRENT, &current_ma);
			...
			input_current = batt_cust_data.usb_charger_current;
			battery_log(BAT_LOG_CRTI,
					"[SDP] Default CC mode charging : %d, input current = %d\r\n",
					current_ma, input_current);
			battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
				 &input_current);
			battery_charging_control(CHARGING_CMD_SET_CURRENT, &current_ma);
			...
			battery_log(BAT_LOG_CRTI,
					"[Others] Default CC mode charging : %d, input current = %d\r\n",
					current_ma, input_current);
			battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
				 &input_current);
			battery_charging_control(CHARGING_CMD_SET_CURRENT, &current_ma);
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
	__batt_parse_node(np, "full_typical_capacity",
		&full_typical_capacity);

	__batt_parse_node(np, "full_design_capacity_umah",
		&full_design_capacity_umah);
#endif
	...
	if ( evdata && evdata->data ) {
		if ( event == FB_EVENT_BLANK ) {
			...
			if( *transition == FB_BLANK_POWERDOWN )
				BMT_status.temperature_delta = 0;
			else if ( *transition == FB_BLANK_UNBLANK )
				BMT_status.temperature_delta = 5;
		}
		battery_log(BAT_LOG_CRTI, "battery common temperature delta:%d\n", BMT_status.temperature_delta);
	}
	return 0;
}
#endif
...
{
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
/*	int chgr_stat_irq, chgr_stat_gpio;
...
/*    node = of_find_compatible_node(NULL, NULL, "mediatek, CHR_STAT-eint");
    if(node)
    {
        of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
        chgr_stat_gpio = ints[0];
        debounce_enable = ints[1];
    }

    chgr_stat_irq = irq_of_parse_and_map(node, 0);
    ret = devm_request_threaded_irq(&(dev->dev), chgr_stat_irq, NULL, battery_common_usbin_interrupt_handler,
                      IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
                      "stat_irq", NULL);
	if(ret < 0){
		battery_log(BAT_LOG_CRTI,"request irq failed\n");
	} */
