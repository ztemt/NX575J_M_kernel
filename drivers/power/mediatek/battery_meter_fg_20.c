...
int Enable_FGADC_LOG = 7; //by Colin for fg log
...
#define ELEMENTS_ROWS       2
struct temp_parameters{
	int key;
	int value;
};
static int __batt_meter_parse_profile_from_dt(struct platform_device *dev,
                void *pbuf, char *property)
{
    ...
    const __be32 *data;
    int num, i;
	struct temp_parameters *map = (struct temp_parameters *)pbuf;
    ...
    if (total_elements % ELEMENTS_ROWS) {
        bm_err("%s table not in multiple of %d, total elements = %d\n",
    ...
    for (i = 0; i < num; i++) {
        map[i].key =  be32_to_cpup(data++);
        map[i].value=  be32_to_cpup(data++);
    }
...
int __batt_meter_init_cust_data_from_dt(struct platform_device *dev)
{
	...
	if (of_property_read_u32(np, "battery_profile_t0_num", &val) == 0) {
		...
		bm_err("Get battery_profile_t0_num: %d\n", batt_meter_table_cust_data.battery_profile_t0_size);
	} else {
		bm_err("Get battery_profile_t0_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.battery_profile_t0 ,"battery_profile_t0");

	if (of_property_read_u32(np, "battery_profile_t1_num", &val) == 0) {
		batt_meter_table_cust_data.battery_profile_t1_size = (int)val;
		bm_err("Get battery_profile_t1_num: %d\n", batt_meter_table_cust_data.battery_profile_t1_size);
	} else {
		bm_err("Get battery_profile_t1_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.battery_profile_t1 ,"battery_profile_t1");

	if (of_property_read_u32(np, "battery_profile_t2_num", &val) == 0) {
		batt_meter_table_cust_data.battery_profile_t2_size = (int)val;
		bm_err("Get battery_profile_t2_num: %d\n", batt_meter_table_cust_data.battery_profile_t2_size);
	} else {
		bm_err("Get battery_profile_t2_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.battery_profile_t2 ,"battery_profile_t2");

	if (of_property_read_u32(np, "battery_profile_t3_num", &val) == 0) {
		batt_meter_table_cust_data.battery_profile_t3_size = (int)val;
		bm_err("Get battery_profile_t3_num: %d\n", batt_meter_table_cust_data.battery_profile_t3_size);
	} else {
		bm_err("Get battery_profile_t3_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.battery_profile_t3 ,"battery_profile_t3");

	/** r profile **/
	if (of_property_read_u32(np, "r_profile_t0_num", &val) == 0) {
		batt_meter_table_cust_data.r_profile_t0_size = (int)val;
		bm_err("Get r_profile_t0_num: %d\n", batt_meter_table_cust_data.r_profile_t0_size);
	} else {
		bm_err("Get r_profile_t0_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.r_profile_t0 ,"r_profile_t0");

	if (of_property_read_u32(np, "r_profile_t1_num", &val) == 0) {
		batt_meter_table_cust_data.r_profile_t1_size = (int)val;
		bm_err("Get r_profile_t1_num: %d\n", batt_meter_table_cust_data.r_profile_t1_size);
	} else {
		bm_err("Get r_profile_t1_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.r_profile_t1 ,"r_profile_t1");

	if (of_property_read_u32(np, "r_profile_t2_num", &val) == 0) {
		batt_meter_table_cust_data.r_profile_t2_size = (int)val;
		bm_err("Get r_profile_t2_num: %d\n", batt_meter_table_cust_data.r_profile_t2_size);
	} else {
		bm_err("Get r_profile_t2_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.r_profile_t2 ,"r_profile_t2");

	if (of_property_read_u32(np, "r_profile_t3_num", &val) == 0) {
		batt_meter_table_cust_data.r_profile_t3_size = (int)val;
		bm_err("Get r_profile_t3_num: %d\n", batt_meter_table_cust_data.r_profile_t3_size);
	} else {
		bm_err("Get r_profile_t3_num failed\n");
	}
	__batt_meter_parse_profile_from_dt(dev,(void *)&batt_meter_table_cust_data.r_profile_t3 ,"r_profile_t3");

	if (of_property_read_u32(np, "soc_flow", &val) == 0) {
		batt_meter_cust_data.soc_flow = (int)val;
		bm_err("Get soc_flow: %d\n", batt_meter_cust_data.soc_flow);
	} else {
		bm_err("Get soc_flow failed\n");
	}

	if (of_property_read_u32(np, "hw_fg_force_use_sw_ocv", &val) == 0) {
		batt_meter_cust_data.hw_fg_force_use_sw_ocv = (int)val;
		bm_err("Get hw_fg_force_use_sw_ocv: %d\n", batt_meter_cust_data.hw_fg_force_use_sw_ocv);
	} else {
	...
	if (of_property_read_u32(np, "q_max_pos_50_h_current", &val) == 0) {
		...
		bm_err("Get q_max_pos_50_h_current: %d\n",
	...
	if (of_property_read_u32(np, "q_max_pos_25_h_current", &val) == 0) {
		...
		bm_err("Get q_max_pos_25_h_current: %d\n",
	...
	if (of_property_read_u32(np, "q_max_pos_0_h_current", &val) == 0) {
		...
		bm_err("Get q_max_pos_0_h_current: %d\n",
	...
	if (of_property_read_u32(np, "q_max_neg_10_h_current", &val) == 0) {
		...
		bm_err("Get q_max_neg_10_h_current: %d\n",
	...
	if (of_property_read_u32(np, "change_tracking_point", &val) == 0) {
		...
		bm_err("Get change_tracking_point: %d\n",
	...
	if (of_property_read_u32(np, "fg_vbat_average_size", &val) == 0) {
		...
		bm_err("Get fg_vbat_average_size: %d\n",
	...
	if (of_property_read_u32(np, "difference_hwocv_rtc", &val) == 0) {
		...
		bm_err("Get difference_hwocv_rtc: %d\n",
	...
	if (of_property_read_u32(np, "difference_hwocv_swocv", &val) == 0) {
		...
		bm_err("Get difference_hwocv_swocv: %d\n",
	...
	if (of_property_read_u32(np, "difference_swocv_rtc", &val) == 0) {
		...
		bm_err("Get difference_swocv_rtc: %d\n",
	...
	if (of_property_read_u32(np, "difference_hwocv_vbat", &val) == 0) {
		...
		bm_err("Get difference_hwocv_vbat: %d\n",
	...
	if (of_property_read_u32(np, "suspend_current_threshold", &val) == 0) {
		batt_meter_cust_data.suspend_current_threshold = (int)val;
		bm_err("Get suspend_current_threshold: %d\n",
	...
	if (of_property_read_u32(np, "vbat_low_power_wakeup", &val) == 0) {
		...
		bm_err("Get vbat_low_power_wakeup: %d\n",
	...
	if (of_property_read_u32(np, "normal_wakeup_period", &val) == 0) {
		...
		bm_err("Get normal_wakeup_period: %d\n",
	...
	if (of_property_read_u32(np, "low_power_wakeup_period", &val) == 0) {
		...
		bm_err("Get low_power_wakeup_period: %d\n",
	...
	if (of_property_read_u32(np, "close_poweroff_wakeup_period", &val) == 0) {
		...
		bm_err("Get close_poweroff_wakeup_period: %d\n",
	...
	if (of_property_read_u32(np, "mtk_enable_aging_algorithm", &val) == 0) {
		...
		bm_err("Get mtk_enable_aging_algorithm: %d\n",
	...
	if (of_property_read_u32(np, "md_sleep_current_check", &val) == 0) {
		...
		bm_err("Get md_sleep_current_check: %d\n",
	...
	if (of_property_read_u32(np, "shutdown_gauge1_xmins", &val) == 0) {
		...
		bm_err("Get shutdown_gauge1_xmins: %d\n",
	...
	if (of_property_read_u32(np, "shutdown_gauge1_mins", &val) == 0) {
		...
		bm_err("Get shutdown_gauge1_mins: %d\n",
...
signed int get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	/* MTK Patch Begin */
	if (g_spm_timer > 10000){
		g_spm_timer = 10000;
	}
	else if (g_spm_timer < 600 && g_sw_vbat_temp > 3600){
		g_spm_timer = 600;
	}
	/* MTK Patch End */
...
signed int battery_meter_get_charging_current(void)
{
	...
	return 0;
#elif defined (CONFIG_NUBIA_PM_COMMON_FEATURE)
	int usb_present = pmic_get_register_value(PMIC_RGS_CHRDET);
	if(usb_present && (BMT_status.bat_charging_state != CHR_ERROR))
		return 0 - (gFG_current/10);
	else
		return 0;
...
kal_bool battery_meter_get_charge_status(void)
{
...
#define SOC_SYNC_PER_SEC		10
#define SOC_INCREASE_PER_SEC	40
#define SOC_DECREASE_PER_SEC 	100
static int is_resumed = 0;
static struct timespec	last_soc_change_time;
static void battery_meter_uisoc2_smooth_tracking(int soc)
{
	int delta_time = 0;
    int soc_changed;
	int last_batt_soc = BMT_status.UI_SOC2;
    int time_since_last_change_sec;
	int usb_present = pmic_get_register_value(PMIC_RGS_CHRDET);
    struct timespec last_change_time = last_soc_change_time;

    /** Power-off Issue */
    if(BMT_status.bat_vol > shutdown_system_voltage && last_batt_soc != 0 && soc == 0 && !usb_present)
    {
        soc = 1;
        bm_notice("[battery smooth]battery voltage is above 3450,set soc to 1 \n");
    }

	calculate_delta_time(&last_change_time, &time_since_last_change_sec);

	if(battery_meter_get_charge_status() == 1) {
		delta_time = time_since_last_change_sec/ SOC_INCREASE_PER_SEC;
	}
	else if( BMT_status.bat_vol < shutdown_system_voltage ) {
		bm_notice("[battery smooth]battery voltage is less than %d, sync soc per %d seconds.\n", shutdown_system_voltage, SOC_SYNC_PER_SEC);
		delta_time = time_since_last_change_sec/ SOC_SYNC_PER_SEC;
	}
	else {
		delta_time = time_since_last_change_sec/ SOC_DECREASE_PER_SEC;
	}
    ...
    {
		/** Charging */
        if( last_batt_soc < soc && battery_meter_get_charge_status() == 1)
            /** Replace the soc when resuming device */
            last_batt_soc = is_resumed ? soc : last_batt_soc + soc_changed;
        /** Discharing */
        else if( last_batt_soc > soc && battery_meter_get_charge_status() == 0)
            /** Replace the soc when resuming device */
            last_batt_soc = is_resumed ? soc : last_batt_soc - soc_changed;
		is_resumed = 0;
    }
	...
	if(BMT_status.UI_SOC2 != last_batt_soc)
	{
		BMT_status.UI_SOC2 = last_batt_soc;
		last_soc_change_time = last_change_time;
	}
	bm_notice("[battery smooth]battery voltage:%d, UI_SOC2:%d, last_batt_soc:%d, soc:%d, soc_changed:%d.\n", BMT_status.bat_vol, BMT_status.UI_SOC2, last_batt_soc, soc, soc_changed);
}
#endif
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_RTC:
		...
		#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
			if(BMT_status.bat_vol > shutdown_system_voltage) {
				rtcvalue = max(rtcvalue, 1);
				bm_notice("[battery smooth]battery voltage:%d is too high, set rtc = %d->1\n", BMT_status.bat_vol, rtcvalue);
			}
		#endif
	...
	case FG_DAEMON_CMD_SET_POWEROFF:
		...
		#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
			if(BMT_status.bat_vol > shutdown_system_voltage){
				bm_notice("[battery smooth]battery voltage:%d is too high. skip power off.\n", BMT_status.bat_vol);
				break;
			}
		#endif
			kernel_power_off();
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
				battery_meter_uisoc2_smooth_tracking(UI_SOC);
#else
...
int wakeup_fg_algo(int flow_state)
{
	...
	#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
		if(flow_state & FG_RESUME) is_resumed = 1;
	#endif
		vfree(fgd_msg);
		return 0;
	} else {
