...
void nubia_enable_spi_clk(struct spi_device * spidev, bool en)
{
	struct spi_master *master;
	struct mt_spi_t *ms;
...
EXPORT_SYMBOL(nubia_enable_spi_clk);
...
static inline void spi_rec_time(const char *str)
{
	...
	if (strncmp(str, "msgs", 4) == 0) {
		...
		snprintf(tmp, sizeof(tmp), "%s:%8lld;", str, rec_time - rec_msg_time[rec_count]);
...
static int mt_spi_setup(struct spi_device *spidev)
{
	...
	if (!((chip_config->pause == PAUSE_MODE_ENABLE)
	      || (chip_config->pause == PAUSE_MODE_DISABLE)) || !((chip_config->cpol == SPI_CPOL_0)
								  || (chip_config->cpol ==
								      SPI_CPOL_1))
...
static int __init mt_spi_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (of_property_read_u32(pdev->dev.of_node, "spi-padmacro", &(ms->pad_macro))) {
			dev_err(&pdev->dev,
				"SPI get pad macro fail failed, using DCT defined SPI pin to set macro\n");
			return -ENODEV;
		}
#if !defined(CONFIG_MTK_CLKMGR)
		...
		if (IS_ERR(ms->clk_main)) {
			dev_err(&pdev->dev,
				"cannot get spi main clock or dma clock. main clk err : %ld .\n",
				PTR_ERR(ms->clk_main));
