...
/* debug log setting */
u8 g_debug_level = ERR_LOG;//DEBUG_LOG;
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
    ...
    gf_dev->pins_pwr_high = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "pwr_high");
    if (IS_ERR(gf_dev->pins_pwr_high)) {
        ret = PTR_ERR(gf_dev->pins_pwr_high);
        gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl miso_spi\n", __func__);
        return ret;
    }
    gf_dev->pins_pwr_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "pwr_low");
    ...
    gf_dev->pins_csb_spi = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "csb_spi");
    if (IS_ERR(gf_dev->pins_csb_spi)) {
        ret = PTR_ERR(gf_dev->pins_csb_spi);
        gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl reset_low\n", __func__);
        return ret;
    }
...
static void gf_power_gpio_cfg(struct gf_device *gf_dev)
{
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
    ...
    if (onoff && enable) {
        ...
        gf_debug(INFO_LOG, "%s, power on!\n", __func__);
        enable = 0;
    } else if (!onoff && !enable) {
        pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_pwr_low);
        gf_debug(INFO_LOG, "%s, power off!\n", __func__);
        enable = 1;
    }
}
static void gf_clk_cfg(struct gf_device *gf_dev)
{
    struct device_node *node = NULL;
    int value;
    node = of_find_compatible_node(NULL, NULL, "mediatek,fingerprint");
    if (node) {
        ...
        gf_debug(DEBUG_LOG, "%s, spi-max-frequency [%d] from dts\n", __func__, value);
        gf_dev->spi->max_speed_hz = value;
    } else {
        gf_debug(ERR_LOG, "%s failed to get device node!\n", __func__);
        return;
    }
...
static void gf_spi_clk_enable(struct gf_device *gf_dev, u8 bonoff)
{
    ...
    static int count = 0;
    if (bonoff && (count == 0)) {
        nubia_enable_spi_clk(gf_dev->spi, true);
        gf_debug(INFO_LOG, "%s, clk enabled!\n", __func__);
        count = 1;
    } else if ((count > 0) && (bonoff == 0)) {
        nubia_enable_spi_clk(gf_dev->spi, false);
        gf_debug(INFO_LOG, "%s, clk disabled!\n", __func__);
        count = 0;
    }
    return;
#endif
...
static int gf_check_9p_chip(struct gf_device *gf_dev)
{
    ...
    do {
        ...
        gf_spi_read_bytes_ree(gf_dev, 0x0142, 4, tmp_buf);
