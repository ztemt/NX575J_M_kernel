...
show_store_prototype(read_capval)// add by baibo 20161117 for capacity test in factory mode
...
static struct attribute *attrs[] = {
	...
	attrify(fifoindex),
	attrify(read_report),
	attrify(read_capval),// add by baibo 20161117 for capacity test in factory mode
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    // begin add by baibo 20161117 for capacity test in factory mode
	retval = synaptics_rmi4_f54_resume_touch_store(dev, attr, cmd, 1);
	if (retval < 0)
		goto exit;

	return count;
    // end add by baibo 20161117 for capacity test in factory mode
...
struct virkey_board_info
{
    ...
    int key_mincap;
    int key_maxcap;
    int virkey_testresult;
};
struct virkey_board_info virkeys[] =
{
{"MENU",1,1,0xffffffff,2778,4399,0},
{"BACK",2,1,0xffffffff,2817,4695,0}
};
...
static ssize_t synaptics_rmi4_f54_read_capval_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
        const char typecmd[] = {'3', 0};
        unsigned int ii;
	    unsigned int jj;
	    unsigned int kk;
	    int tx_num = f54->tx_assigned;
	    ...
	    int count = 0;
	    //struct virkey_cap virkey_temp;
	    //struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	    int virkey_num = sizeof(virkeys)/sizeof(struct virkey_board_info);

        synaptics_rmi4_f54_read_report_store(dev, attr, typecmd, 1);
        printk(KERN_ERR "@@## synaptics_rmi4_f54_read_capval_show\n");

        report_data_16 = (short *)f54->report_data;

		for (ii = 0; ii < tx_num; ii++) {
			for (jj = 0; jj < rx_num; jj++) {
                for (kk = 0; kk < virkey_num; kk++) {
                    if (virkeys[kk].key_valcap != 0xffffffff)
                        continue;

                    if (ii == virkeys[kk].key_tx && jj == virkeys[kk].key_rx) {
                        virkeys[kk].key_valcap = *report_data_16;
                        printk(KERN_ERR "@@## virkeys[kk].key_valcap=%d\n", virkeys[kk].key_valcap);
                    }
                }
        ...
        count += sprintf(buf+count,"%d:", virkey_num);
		for (kk = 0; kk < virkey_num; kk++) {
		    #if 0
		    memset(&virkey_temp, 0, sizeof(struct virkey_cap));
            memcpy(virkey_temp.virkey_name, virkeys[kk].key_name, strlen(virkeys[kk].key_name));
            virkey_temp.virkey_val = virkeys[kk].key_valcap;

            if (virkeys[kk].key_valcap > virkeys[kk].key_mincap && virkeys[kk].key_valcap < virkeys[kk].key_maxcap)
                virkey_temp.virkey_testresult = 1;

            virkey_temp.virkey_minval = virkeys[kk].key_mincap;
            virkey_temp.virkey_maxval = virkeys[kk].key_maxcap;

            memcpy(buf+count, &virkey_temp, sizeof(struct virkey_cap));
            count += sizeof(struct virkey_cap);
            printk(KERN_ERR "@@## name=%s,val=%d,result=%d,min=%d,max=%d\n",virkey_temp.virkey_name,virkey_temp.virkey_val,virkey_temp.virkey_testresult,virkey_temp.virkey_minval,virkey_temp.virkey_maxval);
            #endif

            if (kk != 0)
                count += sprintf(buf+count,"%s ", ",");

            count += sprintf(buf+count,"%s ", virkeys[kk].key_name);
            count += sprintf(buf+count,"%d ", virkeys[kk].key_maxcap);
            count += sprintf(buf+count,"%d ", virkeys[kk].key_mincap);
            count += sprintf(buf+count,"%d ", virkeys[kk].key_valcap);
            if (virkeys[kk].key_valcap > virkeys[kk].key_mincap && virkeys[kk].key_valcap < virkeys[kk].key_maxcap)
                virkeys[kk].virkey_testresult = 1;

            count += sprintf(buf+count,"%d", virkeys[kk].virkey_testresult);
            printk(KERN_ERR "@@## name=%s,val=%d,result=%d,min=%d,max=%d\n",virkeys[kk].key_name,virkeys[kk].key_valcap,virkeys[kk].virkey_testresult,virkeys[kk].key_mincap,virkeys[kk].key_maxcap);

            virkeys[kk].key_valcap = 0xffffffff;
		}
...
static ssize_t synaptics_rmi4_f54_read_capval_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
    return count;
}
