...
#define DELAY_BOOT_READY	200//modify baibo 20170117 from 200 to 10 
#define DELAY_RESET_LOW		20
#define DELAY_UI_READY		200//modify baibo 20170117 from 200 to 10 
...
//for MTK DMA
#define USE_I2C_DMA // modify baibo 20161009
#ifdef USE_I2C_DMA
#include <linux/dma-mapping.h>
...
static int driver_status = 0;// add by baibo for kernel oops issue when driver is not loaded 20161111
...
// begin baibo for keys 20161008
struct pinctrl *synaptics_keys_pinctrl;
struct pinctrl_state *synaptics_keys_eint_as_int, *synaptics_keys_eint_output0, *synaptics_keys_rst_output0, *synaptics_keys_rst_output1;
// end baibo for keys 20161008


static const struct of_device_id synaptics_dt_match[] = {
	{.compatible = "mediatek,synaptics_dsx_keys"},  // modify by baibo 20161008
	{},
};
...
static int of_get_synaptic_platform_data(struct device *dev)
{
    ...
    // begin baibo 20161008
    synaptics_keys_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(synaptics_keys_pinctrl)) {
		ret = PTR_ERR(synaptics_keys_pinctrl);
		dev_err(dev, "fwq Cannot find synaptics_keys pinctrl1!\n");
		return ret;
	}
	synaptics_keys_eint_as_int = pinctrl_lookup_state(synaptics_keys_pinctrl, "synaptics_state_eint_as_int");
	if (IS_ERR(synaptics_keys_eint_as_int)) {
		ret = PTR_ERR(synaptics_keys_eint_as_int);
		dev_err(dev, "fwq Cannot find synaptics_keys pinctrl state_eint_as_int!\n");
		return ret;
	}
	synaptics_keys_eint_output0 = pinctrl_lookup_state(synaptics_keys_pinctrl, "synaptics_state_eint_default");
	if (IS_ERR(synaptics_keys_eint_output0)) {
		ret = PTR_ERR(synaptics_keys_eint_output0);
		dev_err(dev, "fwq Cannot find synaptics_keys pinctrl state_eint_output0!\n");
		return ret;
	}
	synaptics_keys_rst_output0 = pinctrl_lookup_state(synaptics_keys_pinctrl, "synaptics_state_rst_output0");
	if (IS_ERR(synaptics_keys_rst_output0)) {
		ret = PTR_ERR(synaptics_keys_rst_output0);
		dev_err(dev, "fwq Cannot find synaptics_keys pinctrl state_rst_output0!\n");
		return ret;
	}
	synaptics_keys_rst_output1 = pinctrl_lookup_state(synaptics_keys_pinctrl, "synaptics_state_rst_output1");
	if (IS_ERR(synaptics_keys_rst_output1)) {
		ret = PTR_ERR(synaptics_keys_rst_output1);
		dev_err(dev, "fwq Cannot find synaptics_keys pinctrl state_rst_output1!\n");
		return ret;
	}
    // end baibo 20161008
	...
	//printk("tpd_rst_gpio_number %d\n", tpd_rst_gpio_number);
	//printk("tpd_int_gpio_number %d\n", tpd_int_gpio_number);
	return 0;
}
#else
static int of_get_synaptic_platform_data(struct device *dev)
...
static int tpd_irq_registration(void)
{
	...
	node = of_find_matching_node(node, synaptics_dt_match); // modify by baibo 20161008
	if (node) {
        ...
        //printk(KERN_ERR "synaptics_dsx tpd_irq_registration touch_irq = %d\n", touch_irq);
		ret = request_irq(touch_irq, (irq_handler_t) tpd_eint_handler, IRQF_TRIGGER_FALLING,
				"TOUCH_PANEL-eint", NULL); // modify by baibo IRQF_TRIGGER_FALLING 20161008
		if (ret > 0) {
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		if (rmi4_data->fingers_on_2d == false) {
            //printk(KERN_ERR "synaptics_dsx synaptics_rmi4_f1a_report status=%d --1\n",status);
			if (status == 1) {
		    ...
		    //printk(KERN_ERR "synaptics_dsx synaptics_rmi4_f1a_report--2\n");
...
#else
        //printk(KERN_ERR "synaptics_dsx synaptics_rmi4_f1a_report--3\n");
		touch_count++;
		input_report_key(rmi4_data->input_dev,
				f1a->button_map[button],
				status);
		...
		struct synaptics_rmi4_fn *fhandler)
{
    ...
    //printk(KERN_ERR "synaptics_dsx fhandler->fn_number=%02x\n", fhandler->fn_number);
 ...
 */
 ...
 static irqreturn_t tpd_eint_handler(unsigned irq, struct irq_desc *desc)
 {
    //printk(KERN_ERR "synaptics_dsx tpd_eint_handler\n");
...
#ifdef CONFIG_OF_TOUCH
			//tpd_gpio_as_int(1);
			pinctrl_select_state(synaptics_keys_pinctrl, synaptics_keys_eint_as_int); // modify by baibo 20161008
#else
			TPD_GPIO_AS_INT(1);
#endif
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    #if 0 // comment baibo 20161124 for keyboard type input device
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	// begin comment baibo 20161124 for keyboard type input device
...
#endif
    // end comment baibo 20161124 for keyboard type input device

	synaptics_rmi4_set_params(rmi4_data);

    //printk(KERN_ERR "synaptics_rmi4_set_input_dev--1\n");
    ...
    //printk(KERN_ERR "synaptics_rmi4_set_input_dev--2\n");
...
#endif
{
    ...
    //printk(KERN_ERR "synaptics_rmi4_probe--1\n");
    ...
    //printk(KERN_ERR "synaptics_rmi4_probe--2\n");

#if 0 //comment by baibo
#if !defined CONFIG_MTK_LEGACY// add 3.3v begin
	tpd->tpd_dev->of_node = of_find_compatible_node(NULL, NULL, "mediatek,mt_pmic_regulator_supply");
...
#endif
    //printk(KERN_ERR "synaptics_rmi4_probe--3\n");
#ifdef CONFIG_OF_TOUCH
	...
	pinctrl_select_state(synaptics_keys_pinctrl, synaptics_keys_rst_output1); // modify by baibo 20161008

	msleep(DELAY_BOOT_READY);

	//tpd_gpio_output(0,0);// reset pin low
	pinctrl_select_state(synaptics_keys_pinctrl, synaptics_keys_rst_output0); // modify by baibo 20161008
    ...
    //printk(KERN_ERR "synaptics_rmi4_probe--4\n");
    ...
    //printk(KERN_ERR "synaptics_rmi4_probe--5\n");
#ifdef CONFIG_OF_TOUCH
	//tpd_gpio_as_int(1);
	pinctrl_select_state(synaptics_keys_pinctrl, synaptics_keys_eint_as_int); // modify by baibo 20161008
#else
	...
	driver_status = 1;// add by baibo for kernel oops issue when driver is not loaded 20161111

    printk(KERN_INFO "@@## synaptics_rmi4_probe--ok!!\n");
...
// begin modify by baibo for resolve not suspend issue 20161019
void synaptics_dsx_sleep_enable(int enable)
{
    // begin by baibo for kernel oops issue when driver is not loaded 20161111
    if (0 == driver_status)// driver is not loaded
        return;
    // end by baibo for kernel oops issue when driver is not loaded 20161111

    if (1 == enable)
    {
        printk(KERN_ERR "synaptics_rmi4_suspend!\n");
        synaptics_rmi4_suspend(g_dev);
    }else if (0 == enable)
    {
        printk(KERN_ERR "synaptics_rmi4_resume!\n");
        synaptics_rmi4_resume(g_dev);
    }else
    {
        printk(KERN_ERR "synaptics_dsx_sleep_enable error args!!\n");
    }

    return;
}
EXPORT_SYMBOL(synaptics_dsx_sleep_enable);
//end modify by baibo for resolve not suspend issue 20161019
...
static struct i2c_driver tpd_i2c_driver = {
    ...
    .driver = {
           .name = "synaptics_dsx",
#ifdef CONFIG_OF
		   .of_match_table = of_match_ptr(synaptics_dt_match), // modify by baibo 20161008
#endif
		   },
...
//static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("synaptics_dsx", (TPD_I2C_ADDR))}; // comment baibo 20161003
...
{
	//i2c_register_board_info(TPD_I2C_BUS, &i2c_tpd, 1); // comment baibo 20161003

    // begin modify baibo 20161003
    //printk(KERN_ERR "synaptics_rmi4_init\n");
    tpd_local_init();
	// if(tpd_driver_add(&synaptics_rmi4_driver) < 0){
	//	pr_err("Fail to add tpd driver\n");
	//	return -ENODEV;
	//}
    // end modify baibo 20161003
...
{
	//tpd_driver_remove(&synaptics_rmi4_driver); // comment baibo 20161008
	return;
}
