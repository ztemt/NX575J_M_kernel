...
/*add by wuyujing;for NX573J key*/
#define TPD_KEY_COUNT           			2       // 4
#define TPD_KEYS                				{ KEY_MENU, KEY_BACK}
#define TPD_KEYS_DIM            			{{120,1350,20,TPD_BUTTON_HEIGH}, {600,1350,20,TPD_BUTTON_HEIGH}}
#define FT_ESD_PROTECT  					0

/**
 *add by wuyujing,remove TPD_AUTO_UPGRADE, because auto upgrade
 *takes more time,others init function can not execute
 */
...
/* add by wuyujing, for palm lock */
#define KEY_PALM_LOCK 0x2f0
#define FTS_VENDOR_OFILM (0x51)
#define FTS_VENDOR_LIANSI (0x87)
/*******************************************************************************
...
extern int fts_i2c_read_testa(/*struct i2c_client *client,*/ unsigned char *writebuf,int writelen, unsigned char *readbuf, int readlen);
extern int fts_i2c_write_testa(/*struct i2c_client *client,*/ unsigned char *writebuf, int writelen);
...
#ifdef FTS_DBG
#undef FTS_DBG
#define FTS_DBG(fmt, args...) 				printk("[FTS-DEBUG]" fmt, ## args)
#else
...
#ifdef FTS_ERROR
#undef FTS_ERROR
#define FTS_ERROR(fmt, args...) 			printk("[FTS-ERROR]" fmt, ## args)
#else
...
#ifdef FTS_INFO
#undef FTS_INFO
#define FTS_INFO(fmt, args...) 			    printk("[FTS-INFO]" fmt, ## args)
#else
