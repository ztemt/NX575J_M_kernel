...
static unsigned char CTPM_FW[] 				= {
	//#include "ZTEMT_NX541_5x46_ofilm0x51_Ver0x13_D01_20160526_app.i"
	#include "NUBIA_NX573J_ofilm_5x46_Ref_V07_D01_20161121_app.i"
};
#if FT_TP
static unsigned char CTPM_FW_1[] = {
	#include "NUBIA_NX573J_ofilm_5x46_Ref_V08_D01_20170105_app.i"
};
static unsigned char CTPM_FW_2[] = {
	#include "NUBIA_NX573J_liansi_5x46_V52_D01_20170105_app.i"
};
#endif
struct fts_Upgrade_Info fts_updateinfo[] =
{
	...
	{0x54,FTS_MAX_POINTS_10, AUTO_CLB_NONEED,2,  2,  0x54, 0x2c, 20, 2000}, 	//"FT5x46"  // Add by nubia for 10 max points
...
//	u8 reg_val_i2c[2] = {0};
	...
	for (k= 0; k< 1/*FTS_UPGRADE_LOOP*/; k++) 
	{
		...
		if(i_ret == 0)
		{
			pr_err("[FTS] hid change to i2c fail ! \n");
		}
		...
		for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
		{
			...
			if(i_ret < 0)
			{
				pr_err("[FTS] failed writing  0x55 and 0xaa ! \n");
				continue;
			}
			/********* Step 3:check READ-ID ***********************/
			msleep(1);
		...
		pr_err("[FTS][%s] erase app area reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
		/* write bin file length to FW bootloader. */
		auc_i2c_write_buf[0] = 0xB0;
		auc_i2c_write_buf[1] = (u8) ((dw_lenth >> 16) & 0xFF);
		auc_i2c_write_buf[2] = (u8) ((dw_lenth >> 8) & 0xFF);
		...
		if (reg_val[0] != bt_ecc) 
		{
			dev_err(&client->dev, "[FTS]--k=%d, ecc error! fw_ecc=%02x flash_ecc=%02x\n",
						k,reg_val[0],
		...
		else
		{
			...
			pr_err(KERN_WARNING "k=%d,checksum fw_ecc=%X flash_ecc=%X i2c_ecc \n",k,reg_val[0],bt_ecc);//,reg_val_i2c[0]);
			break;
		}
	}
...
int  fts_8606_ctpm_fw_write_pram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	printk("8606 dw_lenth= %d",dw_lenth);
	...
	if (i >= FTS_UPGRADE_LOOP )
    {
		if(pCheckBuffer) kfree(pCheckBuffer);
		return -EIO;
     }
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
		...
		if (i_ret != 0)
		{
			dev_err(&client->dev, "%s:WritePram failed. err.\n",__func__);
			return -EIO;
		}
...
{
	u16 ui_sz = 0;
	unsigned char *ctpm_fw_buf = NULL;

	if (FTS_VENDOR_LIANSI == ucPVendorID) {
		ctpm_fw_buf = CTPM_FW_2;
		ui_sz = sizeof(CTPM_FW_2);
	} else if (FTS_VENDOR_OFILM == ucPVendorID) {
		ctpm_fw_buf = CTPM_FW_1;
		ui_sz = sizeof(CTPM_FW_1);
	}else {
		pr_err("%s invalid VendorID:%d\n", __func__, ucPVendorID);
		/*return fw version 0,do not upgrade*/
		return 0;
	}

	if (ui_sz > 2)
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x36 || fts_updateinfo_curr.CHIP_ID==0x86  || fts_updateinfo_curr.CHIP_ID==0x87)
                return ctpm_fw_buf[0x10a];
	    else if(fts_updateinfo_curr.CHIP_ID==0x58)
                return ctpm_fw_buf[0x1D0A];	//0x010A + 0x1C00
	    else
		return ctpm_fw_buf[ui_sz - 2];//5446 6x06 5336
	}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	pr_err("chip_id:%x uc_tp_fm_ver:%x uc_host_fm_ver:%x\n", chip_id, uc_tp_fm_ver, uc_host_fm_ver);
	...
	if (/*(0x54!=chip_id) ||*/(uc_tp_fm_ver < uc_host_fm_ver)) 	// avoid auto upgrade fail, it will lead to read/write reg fail
	{
		...
		if (i_ret == 0)	
		{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			dev_info(&client->dev, "[FTS] upgrade to new version 0x%x\n",uc_host_fm_ver);
		} 
		else
		{
			#if FT_ESD_PROTECT
