...
static const unsigned char gt1133_sen_map[30] = { 1, 3, 5, 7, 9, 11, 12, 13, 15, 16, 17, 24, 26, 28, 30,
	0, 2, 4, 6, 8, 10, 14, 18, 19, 20, 21, 22, 23, 25, 27
};
...
{
	u8 i, total_num = 40, sen_total_num = 20;
	if (config == NULL) {
	...
	case _GT1143:
		...
		sys.key_number = 0;
		for (i = 0; i < 40; i++) {
			if (config[REG_SEN_START - REG_CONFIG_BASE + i] < total_num) {
				sys.key_number++;
			}
			//DEBUG("config[%d]0x%x, %d",REG_SEN_START+i,config[REG_SEN_START + i],sys.key_number);
		}
		sys.key_number -= sys.sc_driver_num * sys.sc_sensor_num;

		if (config[0x80dc - 0x8050] < sen_total_num) {
		...
		break;
	case _GT1151:
		sys.driver_num = (config[REG_DRV_SEN_CNT - REG_CONFIG_BASE] & 0x1F)
		    + (config[REG_DRV_SEN_CNT + 1 - REG_CONFIG_BASE] & 0x1F);
		...
		sys.key_number = (sys.driver_num * sys.sensor_num - sys.sc_sensor_num * sys.sc_driver_num);
		break;
	default:
		sys.driver_num = (config[REG_DRV_SEN_CNT - REG_CONFIG_BASE] & 0x1F)
		    + (config[REG_DRV_SEN_CNT + 1 - REG_CONFIG_BASE] & 0x1F);
...
int disable_hopping(unsigned char *config, int len, int flag)
{
	...
	do {
		...
		if (memcmp(&config[1], &tmp_config[1], len - 4) == 0) {
			DEBUG("hopping config send success.");
			break;
		}
	} while (++i < 5);
...
void test_to_show(unsigned char *test_result, unsigned short upload_cnt)
{
	int i, ret;
	unsigned char j;
	unsigned char sen_total_num = 20, drv_total_num = 20;
	const u8 *sen_map;

	switch (sys.chip_type) {
	case _GT1133:
		sen_map = gt1133_sen_map;
		sen_total_num = 15;
		drv_total_num = 15;
		break;
	case _GT1143:
	default:
		sen_map = gt1143_sen_map;
		sen_total_num = 20;
		drv_total_num = 20;
		break;
	}
	...
	case _GT1143:
		{
			for (i = 0; i < (upload_cnt * 4); i++) {
				if ((test_result[i] & _CHANNEL_TX_FLAG) == _CHANNEL_TX_FLAG) {
					j = test_result[i] & 0x7f;
					for (ret = 0; ret < drv_total_num; ret++) {
						if (sen_map[sen_total_num + ret] == j) {
							test_result[i] = sen_total_num + ret;
							break;
						}
					}
				} else {
...
void write_test_params(unsigned char *config)
{
	...
	unsigned char sen_total_num = 20, drv_total_num = 20, total_num = 40;
	const u8 *sen_map;
	...
	case _GT1143:
		{
			...
			for (i = 0, k = 0, j = 0; i < 40; i++) {
				if (config[sen_offest + i] < sen_total_num) {
					if ((i % 10) < sys.sensor_num) {
						data[j++] = sen_map[config[sen_offest + i]];
					} else {
						tmp[k++] = sen_map[config[sen_offest + i]];
					}
				}
			}
			memcpy(&data[j], tmp, k);
			...
			for (i = 0, j = 0, k = 0; i < 40; i++) {
				if ((config[sen_offest + i] >= sen_total_num) && (config[sen_offest + i] < total_num)) {
					if ((i % 10) < sys.sensor_num) {
						data[sys.max_driver_num + j++] = sen_map[config[sen_offest + i]];
					} else {
						tmp[k++] = sen_map[config[sen_offest + i]];
					}
				}
...
unsigned int my_getline(char *buf, int size, FILE * fp)
{
	...
	while (i < size) {
		...
		if (bytes < 0) {
			...
			DEBUG("fread bytes err:%zd", bytes);
...
int init_chip_type(void)
{
	...
	} else if (!strncmp((const char *)gt1x_version.product_id, "1133", 4)) {
...
static ssize_t gtp_sysfs_openshort_show(struct device *dev, struct device_attribute *attr, char *buff)
{
	...
	if (ret < 0) {
		len += sprintf((char *)&buff[len], "test error code 0x%zu", ret);
	}else if (ret == 0) {
		len += sprintf((char *)&buff[len], "0");
	}else {
...
s32 gtp_test_sysfs_init(void)
{
	...
	ret = sysfs_create_file(&gt1x_i2c_client->dev.kobj, &dev_attr_openshort.attr);
...
{
	sysfs_remove_file(&gt1x_i2c_client->dev.kobj, &dev_attr_openshort.attr);
}
