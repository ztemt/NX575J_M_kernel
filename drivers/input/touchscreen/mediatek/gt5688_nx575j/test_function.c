...
static unsigned char _check_area_accord(void)
{
	...
	pr_err("sys.sc_sensor_num:%d  sys.sc_driver_num:%d\n", sys.sc_sensor_num, sys.sc_driver_num);
	for (i = 0; i < sys.sc_sensor_num; i++) {
		for (j = 0; j < sys.sc_driver_num; j++) {
			...
			//pr_err("accord_temp:%ld  accord_limit[index]:%ld, i = %d j = %d\n", accord_temp, accord_limit[index], i, j);
...
//  if ((check_types & _TEST_RESULT_SAVE) != 0)
	...
	DEBUG("The %d group rawdata test end! samping_set_number:%d\n", current_data_index, samping_set_number);
...
static s32 _alloc_test_memory(int b_size)
{
	...
	global_large_buf = (u8 *) malloc(70 * b_size + 4 * 1024);
	...
	memset(global_large_buf, 0, 70 * b_size + 4 * 1024);
