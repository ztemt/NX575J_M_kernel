...
int palm_sleep_enable = 0;//begin baibo 20161109 for palm sleep
...
s32 gesture_event_handler(struct input_dev * dev)
{
    ...
    // begin baibo 20161109 for gesture wakeup 0XCC-double click wakeup
    if (0xCC == ges_type)
    {
        key_code = KEY_GES_DOUBLECLICK;
    }
    // end baibo 20161109 for gesture wakeup
...
static ssize_t gt1x_easy_wakeup_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    u32 cmd;
    if (sscanf(buf, "%x", &cmd) == 1)
    {
        GTP_DEBUG( "gt1x_easy_wakeup_gesture_store write cmd %x \n",cmd);
        if(cmd == 1)
        {
            gesture_enabled = 1;
            memset(gestures_flag, 0xFF, sizeof(gestures_flag));
    ...
    {
        GTP_DEBUG("gt1x_easy_wakeup_gesture_store cmd error\n");
		return -EINVAL;
    }
}

static ssize_t gt1x_eeasy_palm_sleep_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    u32 cmd;
    if (sscanf(buf, "%x", &cmd) == 1)
    {
        GTP_DEBUG( "gt1x_eeasy_palm_sleep_store write cmd %x \n",cmd);
        if(cmd == 1)
        {
            palm_sleep_enable = 1;
        }
    ...
    {
        GTP_DEBUG("gt1x_eeasy_palm_sleep_store cmd error\n");
		return -EINVAL;
    }
}

/*add by wuyujing, show tp version,start*/
static ssize_t gt1x_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *ptr = buf;
    char temp_data[GTP_CONFIG_MAX_LENGTH] = { 0 };
    ssize_t ret = 0;
    unsigned char cfg_ver[1] = {0};

    ret = gt1x_i2c_read(GTP_REG_VERSION, temp_data, 12);
    if (ret < 0) {
        GTP_ERROR("Read Version error:%zd!", ret);
        return ret;
    }

    ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, cfg_ver, 1);
    if (ret < 0) {
        GTP_ERROR("Read CFG version error:%zd", ret);
        return ret;
    }

    ptr += sprintf(ptr, "ProductID:GT%c%c%c%c ", temp_data[0], temp_data[1], temp_data[2], temp_data[3]);
    ptr += sprintf(ptr, "PatchID:%02X%02X\n", temp_data[4], temp_data[5]);
    ptr += sprintf(ptr, "MaskID:%02X%02X ", temp_data[7], temp_data[8]);
    ptr += sprintf(ptr, "SensorID:%02X", temp_data[10] & 0x0F);
    ptr += sprintf(ptr, "CfgID:%02X\n", cfg_ver[0] & 0xFF);

    return (ptr - buf);
}

static DEVICE_ATTR(gt1x_version, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IXOTH, gt1x_version_show, NULL);
/*add by wuyujing, show tp version,end*/

static DEVICE_ATTR(easy_wakeup_gesture, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IXOTH, NULL, gt1x_easy_wakeup_gesture_store);
static DEVICE_ATTR(easy_palm_sleep, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IXOTH, NULL, gt1x_eeasy_palm_sleep_store);


static struct attribute *fts_attributes[] = {
    &dev_attr_easy_wakeup_gesture.attr,
    &dev_attr_easy_palm_sleep.attr,
    &dev_attr_gt1x_version.attr,
	NULL
};

static struct attribute_group gt1x_attribute_group = {
	.attrs = fts_attributes
};

int gt1x_create_sysfs(void)
{
	int err;
	err = sysfs_create_group(&gt1x_i2c_client->dev.kobj, &gt1x_attribute_group);
	if (0 != err)
	{
		GTP_ERROR("ERROR: sysfs_create_group() failed!!\n");
		sysfs_remove_group(&gt1x_i2c_client->dev.kobj, &gt1x_attribute_group);
		return -EIO;
	}
	else
	{
