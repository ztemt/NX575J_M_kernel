...
{
	/* modify by wuyujing,access file when user version */
	gt1x_debug_proc_entry = proc_create(GT1X_DEBUG_PROC_FILE, 0666, NULL, &gt1x_debug_fops);
...
s32 gt1x_init_panel(void)
{
	...
	ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, ic_version, 1);
	if (ret < 0){
		GTP_ERROR("Read ic version failed!");
		return ret;
	}else if (ic_version[0] > 0x80){
		GTP_INFO("Send ic version 0xFF");
		gt1x_config[0] = 0xff;
	}
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				/*add by wuyujing, rotation 180 degree, start*/
				input_x = 719 - input_x;
				/*disable by nubia for new configuration*/
				/*input_y = 1279 - input_y;*/
				/*add by wuyujing, rotation 180 degree, end*/
...
struct smart_cover_device{
    int enabled;
    int state; // 0:cover faraway other:near
...
/* Begin add by Nubia for smartcover when tp suspend on 20170112 */
static int gt1x_touch_mode_change = 0;
/* End add by Nubia for smartcover when tp suspend on 20170112 */
...
static ssize_t smart_cover_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	/*Modified by Nubia for enabling smartcover with value 2*/
	/*if (!dev || !dev->enabled || s > 1 || s == dev->state) */
	if (!dev || !dev->enabled || s == dev->state) {
		return count;
	}
...
static struct kobj_attribute sc_attr = 
    __ATTR(state, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, smart_cover_show, smart_cover_store);// modify baibo 20161122 for cover
...
/* init auxiliary  node and functions */
    ...
    /* add by wuyujing, openshort test */
    gtp_test_sysfs_init();
