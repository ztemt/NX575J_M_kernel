...
#define GPIO_TP_3P0_EN 	(GPIO57 | 0x80000000) 
...
int gt1x_power_switch(s32 state)
{
...
#endif
		mt_set_gpio_mode(GPIO_TP_3P0_EN, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_TP_3P0_EN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_TP_3P0_EN, GPIO_OUT_ONE);
		power_state = 1;
	}
	break;
	...
	if (power_state == 1) {
		...
		mt_set_gpio_mode(GPIO_TP_3P0_EN, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO_TP_3P0_EN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_TP_3P0_EN, GPIO_OUT_ZERO);
		power_state = 0;
	}
	break;
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_GES_DOUBLECLICK);//add baibo 20161109 for doubleclick wakeup
	input_set_capability(tpd->dev, EV_KEY, KEY_GES_PALMSLEEP);//add baibo 20161109 for palm sleep
#endif
...
static int tpd_event_handler(void *unused)
{
	...
	s32 ret = 0;
	u8 point_data[11] = { 0 };
	u8 large_detect = 0;//add baibo 20161109 for palm sleep
	...
	do {
        ...
        //begin baibo 20161109 for palm sleep
        large_detect = finger & 0x40;
		if (large_detect != 0 && palm_sleep_enable != 0){
            GTP_DEBUG("report palm sleep event!!\n");

            input_report_key(tpd->dev, KEY_GES_PALMSLEEP, 1);
	        input_sync(tpd->dev);
	        input_report_key(tpd->dev, KEY_GES_PALMSLEEP, 0);
	        input_sync(tpd->dev);
		}
		//end baibo 20161109 for palm sleep
