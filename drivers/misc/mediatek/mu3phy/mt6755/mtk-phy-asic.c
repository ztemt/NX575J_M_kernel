...
//add by houpeng-- change VOL_LEVEL used by USB phy related to USB eye diagram test
#ifdef CONFIG_MTK_USB_VOL_LEVL_SET
#define VOL_LEVL CONFIG_MTK_USB_VOL_LEVL_SET
#else
#define VOL_LEVL 6
#endif
//add end
...
PHY_INT32 phy_init_soc(struct u3phy_info *info)
{
	...
	//add by houpeng-- change VOL_LEVEL used by USB phy related to USB eye diagram test
	U3PhyWriteField32((phys_addr_t) (uintptr_t) U3D_USBPHYACR1, RG_USB20_VRT_VREF_SEL_OFST, RG_USB20_VRT_VREF_SEL, VOL_LEVL);//添加此行，参数5最大值是7
	U3PhyWriteField32((phys_addr_t) (uintptr_t) U3D_USBPHYACR1, RG_USB20_TERM_VREF_SEL_OFST, RG_USB20_TERM_VREF_SEL, VOL_LEVL); //添加此行，参数5最大值是7
	//add end
