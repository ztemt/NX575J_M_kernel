...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno , 1 , CAM_CAL_DRVNAME)) {
		CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] Register device no failed\n");
	...
	if (NULL == g_pCAM_CAL_CharDrv) {
		...
		CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] Attatch file operation failed\n");
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] Attach I2C\n");
	/* spin_lock_init(&g_CAM_CALLock); */

	/* get sensor i2c client */
	spin_lock(&g_CAM_CALLock); /* for SMP */
	g_pstI2Cclient = client;
	g_pstI2Cclient->addr = BRCB032GWZ_3_DEVICE_ID >> 1;
	spin_unlock(&g_CAM_CALLock); /* for SMP */

	CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] g_pstI2Cclient->addr = 0x%8x\n", g_pstI2Cclient->addr);
	/* Register char driver */
	i4RetValue = RegisterCAM_CALCharDrv();

	if (i4RetValue) {
		CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] register char device failed!\n");
		return i4RetValue;
	}


	CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] Attached!!\n");
	return 0;
}
...
static int __init CAM_CAL_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALDB("failed to register BRCB032GWZ_3_CAM_CAL driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALDB("failed to register BRCB032GWZ_3_CAM_CAL driver, 2nd time\n");
		return -ENODEV;
	}
