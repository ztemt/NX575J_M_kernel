...
static struct i2c_board_info i2cDev1 = { I2C_BOARD_INFO(CAM_CAL_I2C_DEV1_NAME, 0xA0 >> 1)};
...
static int __init cam_cal_drv_init(void)
{
	...
	//modified by kangxiong to avoid dts read failed -----start
	if (node1) {
		CAM_CALDB("kangxiong:get busNum from dts \n");
		of_property_read_u32(node1, "main_bus", &g_busNum[BUS_ID_MAIN]);
		of_property_read_u32(node1, "sub_bus", &g_busNum[BUS_ID_SUB]);
	}else{
		CAM_CALDB("kangxiong:init bus num \n");
		g_busNum[BUS_ID_MAIN] = 2;//IIC Bus for main camera
		g_busNum[BUS_ID_SUB] = 1;//IIC Bus for sub camera
	}
	//modified by kangxiong to avoid dts read failed -----start
