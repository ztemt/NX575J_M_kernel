...
//larks add for sec chip flag
int sec_schip_enabled(void);
//extern char *saved_command_line;//larks add for ddr type send to kernel debug interface
static int flag = -1;
enum {
	BM_UNINIT = 0,
	BM_INITIALIZING = 1,
...
static int  ddr_type_setup(char *param)
{
	int magic_num = 0;
...
early_param("ddr_type", ddr_type_setup);
//larks add for ddy_type start 
static int ddy_type_proc_show(struct seq_file *p, void *v)
{
	//int ret = -1;
//	char *ptr;
	//int a = -1;
	
//	ptr = strstr(saved_command_line,"ddr=");
//	ptr += strlen("ddr=");
	
//	ret = simple_strtol(ptr,NULL,10);
	pr_err("larks enter ddr type\r\n");
	
	seq_puts(p, "\n\r the ddr type is   : ");
	
	pr_err("larks the sec flag is %d\r\n",flag);
	switch (flag) {
	case 8:
		seq_puts(p, "8\n");
		break;
...
//larks add for ddy_type end

//larks add for sec flag start 
static int sec_flag_proc_show(struct seq_file *p, void *v)
{
	int ret = -1;
	seq_puts(p, "\n\rMTK SEC MODE : ");
	...
	switch (ret) {
	case 0:
		seq_puts(p, "SEC FLAG NSC\n");
		break;
	case 1:
		seq_puts(p, "SEC FLAG SC!!\n");
		break;
...
//larks add for sec flag start 
static int sec_flag_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, sec_flag_proc_show, NULL);
}

static const struct file_operations sec_flag_proc_fops = {
	.open = sec_flag_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
//larks add for ddr_type start 
static int ddr_type_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, ddy_type_proc_show, NULL);
}
static const struct file_operations ddr_type_proc_fops = {
	.open = ddr_type_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

 //larks add for ddr_type end
static int __init boot_common_core(void)
{
	...
	if (NULL == proc_create_data("boot_mode", S_IRUGO, NULL, &boot_mode_proc_fops, NULL))
       ...
       //larks add for sec flag start
	if (NULL == proc_create_data("sec_flag", S_IRUGO, NULL, &sec_flag_proc_fops, NULL))
		pr_err("larks create sec_flag fail");
	//larks add for sec flag end
	       //larks add for ddr_type start
	if (NULL == proc_create_data("ddr_type", S_IRUGO, NULL, &ddr_type_proc_fops, NULL))
		pr_err("larks create ddr_type fail");
	//larks add for ddr_type end
