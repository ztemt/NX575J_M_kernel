...
* add the hardware  version judge for mag direction by LiuQIang of nubia
*/
...
struct mag_hw *get_mag_dts_func(const char *name, struct mag_hw *hw)
{
	...
	if (name == NULL)
		return NULL;
        hw_ver=kmalloc(sizeof(char)*4,GFP_KERNEL);
        if(!hw_ver){
            printk(KERN_ERR "kmalloc failed in fun:%s\n",__func__);
            return NULL;
        }
	...
	if (node) {
		...
		ret = of_property_read_u32_array(node , "direction", direction, ARRAY_SIZE(direction));
		if (ret == 0){
                    if(strcmp(hw_ver,"MB_A")==0){
                        hw->direction = direction[0];
                    }else if(strcmp(hw_ver,"MB_B")==0){
                        hw->direction = direction[1];
                    }else {
                        hw->direction = direction[0];
                    }
                }
