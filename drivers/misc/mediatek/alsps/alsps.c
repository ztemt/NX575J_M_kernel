...
#define NUM_ALSPS_NAME 8
static char alsps_driver_name[NUM_ALSPS_NAME];
#endif
...
/*add by LiuQiang report 3/10 start*/
#ifdef CONFIG_MTK_TMD2725X_NEW
    if(!strncmp(alsps_driver_name,"TMD2725X",NUM_ALSPS_NAME)){
	if (value == 0){
	    value = 10;
	}else if (value == 1){
	    value = 3;
...
#endif
/*add by LiuQiang report 3/10 end*/
...
static int ps_enable_data(int enable)
{
	...
	if (1 == enable) {
		...
		#if 0 // comment baibo 20170209 for cancel alsps.c driver auto report once ps data when ps enable
...
static int alsps_real_driver_init(void)
{
	...
	for (i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++) {
		...
		if (0 != alsps_init_list[i]) {
			...
			if (0 == err) {
#ifdef CONFIG_MTK_TMD2725X_NEW
		strcpy(alsps_driver_name,alsps_init_list[i]->name);
#endif
