...
/*fix psensor thres  value too small start*/
int APDS9930_CMM_PPCOUNT_VALUE = 0x07;
int APDS9930_CMM_CONTROL_VALUE = 0x68;
/*fix psensor thres  value too small end*/
int ZOOM_TIME = 4;
#ifdef CONFIG_MTK_TMD2725X_NEW
int ag_err=0;
#endif
...
int APDS9930_read_als(struct i2c_client *client, u16 *data)
{
	...
	/*APS_LOG("AARON:c0_value=%d, c0_nf=%d, als_modulus=%d\n", c0_value, c0_nf, obj->als_modulus);*/
	...
	/*APS_LOG("AARON:c1_value=%d, c1_nf=%d, als_modulus=%d\n", c1_value, c1_nf, obj->als_modulus);*/

	if ((c0_value > c1_value) && (c0_value < 50000)) {
		atio = (c1_nf * 100) / c0_nf;

		/*APS_LOG("AARON:atio = %d\n", atio);*/
		if (atio < 17) {
			*data = (((13 * c0_nf - 24 * c1_nf) /10000)*3)/2;
		}else if (atio >= 17 && atio <30) {
		...
		} else if (atio >= 45 && atio < 50) {
			*data = (6 * c0_nf - 10 * c1_nf)  / 30000;
		} else if (atio >= 50 && atio < 56) {
			*data = (6 * c0_nf - 10 * c1_nf)  / 40000;
		} else
			*data = 0;
	...
	/*APS_LOG("AARON:APDS9930_read_als als_value_lux = %d\n", *data);*/
	return 0;
...
static void APDS9930_irq_work(struct work_struct *work)
{
	...
	if (err) {
		...
		printk(KERN_DEBUG"APDS9930_irq_work rawdata ps=%d als_ch0=%d!\n", obj->ps, obj->als);

		printk(KERN_DEBUG"APDS9930 int top half time = %lld\n", int_top_time);

		if (obj->als > 52428) {
			printk(KERN_DEBUG"APDS9930_irq_work ALS too large may under lighting als_ch0=%d!\n",
				obj->als);
	        als_ch0_full=1;
	        goto EXIT_ERR;
		}
...
static int ps_enable_nodata(int en)
{
	...
	APS_LOG("APDS9930_obj ps enable value = %d\n", en);
...
#ifdef CONFIG_MTK_TMD2725X_NEW
   for(num=0;num<5;num++){
       databuf[0]=APDS9930_CMM_DEVICEID;
       APDS9930_i2c_master_operate(client, databuf, 0x101, I2C_FLAG_READ);
       if(databuf[0]==57){
           printk("APDS9930 DEVICEID:%d\n",databuf[0]);
           break;
       }else{
           if(num>=4){
               ag_err=-6;
               printk("device of apds9930 ic is not this system!");
               return ag_err;
           }
           continue;
       }
