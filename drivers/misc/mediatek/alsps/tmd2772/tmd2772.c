...
static int tmd2772_init_client_call(struct i2c_client *client);
...
#define GPIO_ALS_EINT_PIN         (GPIO105|0x80000000)
...
static int tmd2772_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
     int i=0 ,err = 0,j = 0;
     u16 data[21];
     u16 sum=0,data_cali=0,max=0,min=0;
     mdelay(15);
     printk("enter %s\n",__FUNCTION__);
	 for(i = 0;i<5;i++)
		{
			//mdelay(5);//50
			err = (int)tmd2772_read_ps(client,&data[i]);
			if(err)
			{
	 ...
	 if(i == 5)
		{
			data_cali = sum/3;
			printk("tmd2772_read_data_for_cali data = %d\n",data_cali);
			//if(data_cali>600)
			//return -1;
			if(data_cali<=800)
			{
				ps_data_cali->close =data_cali+150;
				ps_data_cali->far_away = data_cali+100;
			...
			ps_cali.valid = 1;
			printk("tmd2772_read_data_for_cali close  = %d\t,far_away = %d\t,valid = %d\n",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid);
...
//	u8 reg_value[1];
	...
	tmd2772_init_client_call(client);
	err = tmd2772_read_data_for_cali(client,&ps_cali_temp);
	if(err)
	{
	   printk("%s():tmd2772_read_data_for_cali error!\n",__func__);
	   return -1;
	}
...
//get adc channel 1 value
	...
	c0_nf = obj->als_modulus*c0_value*2;
...
#if 0
static int tmd2772_read_cali_val(struct i2c_client *client,int n)
{
	int i=0,err=0 ,j = 0;

	u16 data[21],sum,data_cali=0;
	printk("enter %s\n",__FUNCTION__);
	sum = 0;
	mdelay(10);
	for(i = 0;i<n;i++)
...
EXIT_ERR:
	APS_ERR("tmd2772_read_cali_val fail\n");
	return -1;

}

static int tmd2772_offset(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
	u8 databuf[2];
	u16 data_cali=0;
	int err=0;
	...
	data_cali = tmd2772_read_cali_val(client,10);
	...
	if((data_cali>=0)&&(data_cali<50))
	{
		...
		data_cali = tmd2772_read_cali_val(client,10);
	}else if((data_cali>=600)&&(data_cali<800)){
		databuf[0] = TMD2772_CMM_OFFSET;
		databuf[1] = 0X00|0x50;
		err = i2c_master_send(client, databuf, 0x2);
		if(err <= 0)
	...
	if((data_cali>=200)&&(data_cali<600))
	{
		ps_data_cali->close =data_cali+150;
		ps_data_cali->far_away = data_cali+100;
		ps_data_cali->valid =1;
	...
	ps_cali.far_away= ps_data_cali->far_away;
	ps_cali.valid = 1;
	printk("tmd2772_offset close  = %d\t,far_away = %d\t,valid = %d\n",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid);
    return err;
}
#endif
...
static int tmd2772_read_data_for_cali_first(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
     int i=0 ,err = 0;//j = 0;
     u16 data[21];
     u16 sum=0,data_cali=0,max=0,min=0;
...
static int tmd2772_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->als_modulus = (400*100*1)/(1*150);//(1/Gain)*(400/Tine), this value is fix after init ATIME and CONTROL register value
	...
	tmd2772_init_client_call(client);
	err = tmd2772_read_data_for_cali_first(client,&ps_cali_temp);
	if(err)
	{
	   printk("%s():tmd2772_read_data_for_cali error!\n",__func__);
	   goto exit;
	...
	printk("tmd2772_init_client() OK!\n");

#if 0
	tmd2772_init_client_call(client);

	err =tmd2772_offset(client,&ps_cali_temp);
	printk("offset err =%d\n",err);
	if(err)
	{
		 printk("%s():tmd2772_offset error!\n",__func__);
		 return -1;
	}
	printk("tmd2772_offset() OK!\n");
#endif
