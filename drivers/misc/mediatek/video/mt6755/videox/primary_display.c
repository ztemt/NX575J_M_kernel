...
/*modify by wuyujing,support cabc*/
...
int _set_acl_by_cmdq(unsigned int enbale)
{
	int ret = 0;
	cmdqRecHandle cmdq_handle_acl = NULL;
	ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &cmdq_handle_acl);
	...
	if (primary_display_is_video_mode()) {
		cmdqRecReset(cmdq_handle_acl);
		disp_lcm_set_acl(pgc->plcm, cmdq_handle_acl, enbale);
		_cmdq_flush_config_handle_mira(cmdq_handle_acl, 1);
		DISPCHECK("[BL]_set_cabc_by_cmdq ret=%d\n", ret);
	} else {
		cmdqRecReset(cmdq_handle_acl);
		cmdqRecWait(cmdq_handle_acl, CMDQ_SYNC_TOKEN_CABC_EOF);
		_cmdq_handle_clear_dirty(cmdq_handle_acl);
		_cmdq_insert_wait_frame_done_token_mira(cmdq_handle_acl);
		disp_lcm_set_acl(pgc->plcm, cmdq_handle_acl, enbale);
		cmdqRecSetEventToken(cmdq_handle_acl, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
		cmdqRecSetEventToken(cmdq_handle_acl, CMDQ_SYNC_TOKEN_CABC_EOF);
		_cmdq_flush_config_handle_mira(cmdq_handle_acl, 1);
		DISPCHECK("[BL]_set_acl_by_cmdq ret=%d\n", ret);
	}

	cmdqRecDestroy(cmdq_handle_acl);
	cmdq_handle_acl = NULL;
	return ret;
}


int primary_display_setacl(unsigned int enbale)
{
	static unsigned int last_enable = -1;
	int ret = 0;
	...
	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State set acl invald\n");
		ret = -1;
	} else {
		primary_display_idlemgr_kick(__func__, 0);
		if (primary_display_cmdq_enabled()) {
			if (primary_display_is_video_mode()) {
				ret = disp_lcm_set_acl(pgc->plcm, NULL, enbale);
			} else {
				ret = _set_acl_by_cmdq(enbale);
			}
		}
		last_enable = enbale;
	}
...
int primary_display_getacl(unsigned int *level)
{
	int ret = 0;

	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State get acl invald\n");
		ret = -1;
	} else {
		ret = disp_lcm_get_acl(pgc->plcm, NULL, level);
	}
