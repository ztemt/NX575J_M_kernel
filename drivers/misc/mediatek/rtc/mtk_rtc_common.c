...
static int print_suspend_time(struct device *dev)
{
    ...
    all_wake_time += diff;
        getnstimeofday(&ts);
    printk("I have work %d seconds all_wake_time %lu seconds %d-%02d-%02d %02d:%02d:%02d.%03lu UTC\n",
        diff,all_wake_time,tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec);
...
static int print_resume_time(struct device *dev)
{
    ...
    all_sleep_time += diff;
        getnstimeofday(&ts);
    printk("I have sleep %d seconds all_sleep_time %lu seconds %d-%02d-%02d %02d:%02d:%02d.%03lu UTC\n",
        diff,all_sleep_time,tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec);

    return 0;
}

static const struct dev_pm_ops mtk_rtc_pm_ops = {
