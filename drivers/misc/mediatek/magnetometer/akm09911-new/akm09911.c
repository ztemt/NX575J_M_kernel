...
#define AKM_Device_AK9916 //add  ak9916 function by qliu4244
#undef  AKM_Device_AK9916
...
#ifdef AKM_Device_AK9916
void  AK9916_TO_AK9911(char *rxData, int length){
       char ak9916_data[SENSOR_DATA_SIZE-1]={0};
       long temp_data;
       int num=1;
       for(num=1;num<=length;num+=2){
	     temp_data=(rxData[num]|rxData[num+1]<<8)/4;
	     ak9916_data[num]=0xFF&temp_data>>8;
	     ak9916_data[num+1]=0xFF&temp_data;
       }
       for(num=1;num<=length;num++)
	     rxData[num]=ak9916_data[num];
}
#endif
...
#ifdef AKM_Device_AK9916
	AK9916_TO_AK9911(&rbuf[1], SENSOR_DATA_SIZE - 1);
#endif
	#endif
	if (ret < 0) {
