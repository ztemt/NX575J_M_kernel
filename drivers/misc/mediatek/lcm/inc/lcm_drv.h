...
typedef enum {
	CABC_OFF = 23,	/*Off Mode*/
	CABC_UI,	/*UI Image Mode*/
	CABC_STILL,	/*Still Image Mode*/
	CABC_MOVING	/*Moving Image Mode*/
}LCM_CABC;
...
typedef struct {
	...
	/*add by wuyujing, support cabc interface*/
	void (*set_cabc)(unsigned int level);
	void (*get_cabc)(unsigned int *level);
	void (*is_lcmon)(unsigned int *isok);
	void (*set_acl_cmdq)(void *handle, unsigned int level);
	void (*get_acl)(unsigned int *level);
} LCM_DRIVER;
