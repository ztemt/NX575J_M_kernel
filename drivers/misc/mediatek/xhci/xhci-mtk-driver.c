...
u32 xhci_debug_level = K_ALET | K_CRIT | K_ERR | K_WARNIN | K_INFO;
...
static bool mtk_is_charger_4_vol(void)
{
    ...
    if(usb_psy)
    {
        ret = usb_psy->get_property(usb_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE, &val);
        vol = val.intval;
    }
#else
...
#ifdef CONFIG_NUBIA_GENERAL_CHARGE_COMMON
static int nubia_set_pmic_otg_mode(int val)
{
    union power_supply_propval ret = {val,};
    struct power_supply	*batt_psy = 0;

    batt_psy = power_supply_get_by_name("usb");

	if (batt_psy){
		batt_psy->set_property(batt_psy, POWER_SUPPLY_PROP_USB_OTG, &ret);
		return ret.intval;
	}
	return -EINVAL;
}
...
{
	mtk_xhci_mtk_printk(K_CRIT, "mtk_enable_otg_mode\n");
#if defined(CONFIG_MTK_BQ25896_SUPPORT)
	bq25890_otg_en(0x01);
	bq25890_set_boost_ilim(0x03);	/* 1.3A */
#elif defined(CONFIG_NUBIA_GENERAL_CHARGE_COMMON)
...
#endif
#if defined(CONFIG_MTK_RT9466_SUPPORT)
	set_chr_enable_otg(0x1);
	set_chr_boost_current_limit(1300);
#endif
//added by houpeng.  support charger IC BQ24296 for NX573J
#if defined(CONFIG_MTK_BQ24296_SUPPORT)
	bq24296_set_otg_config(0x01);
	bq24296_set_boost_lim(0x01);
#endif
//add end
}
...
{
	mtk_xhci_mtk_printk(K_CRIT, "mtk_disable_otg_mode\n");
#if defined(CONFIG_MTK_BQ25896_SUPPORT)
	bq25890_otg_en(0x0);
#elif defined(CONFIG_NUBIA_GENERAL_CHARGE_COMMON)
    nubia_set_pmic_otg_mode(0x00);
#ifdef CONFIG_MTK_LEGACY
    mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN, GPIO_OTG_DRVVBUS_PIN_M_GPIO);
    mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN, GPIO_OUT_ZERO);
#else
    mt_set_gpio_mode(0x80000043, 1);
    mt_set_gpio_out(0x80000043, GPIO_OUT_ZERO);
#endif
#elif defined(CONFIG_MTK_OTG_PMIC_BOOST_5V)
	mtk_disable_pmic_otg_mode();
#endif
...
{
	int ret = 0;
      mtk_xhci_mtk_printk(K_CRIT, "mtk_xhci_driver_load\n");
...
void mtk_xhci_mode_switch(struct work_struct *work)
{
	...
	int ret = 0;
	mtk_xhci_mtk_printk(K_CRIT, "mtk_xhci_mode_switch\n");
...
{
	mtk_xhci_mtk_printk(K_CRIT, "mtk_is_host_mode vbus_on=%d, mtk_idpin_cur_stat=%d\n", vbus_on, mtk_idpin_cur_stat);
...
{
	int ret = 0;
	mtk_xhci_mtk_printk(K_CRIT, "typec_otg_enable\n");
	mtk_idpin_cur_stat = IDPIN_IN_HOST;
...
{
	mtk_xhci_mtk_printk(K_CRIT, "typec_otg_disable\n");
	mtk_xhci_disPortPower();
	/* USB PLL Force settings */
	usb20_pll_settings(true, false);
	mtk_xhci_driver_unload();
