...
int flashlight_gpio_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(flashlight_pinctrl)) {
		pr_err("Cannot find flashlight pinctrl!");
	...
	if (IS_ERR(flashlight_hwen_high)) {
		...
		pr_err("%s : init err, flashlight_hwen_high\n", __func__);
	}
	...
	if (IS_ERR(flashlight_hwen_low)) {
		...
		pr_err("%s : init err, flashlight_hwen_low\n", __func__);
	}
	...
	if (IS_ERR(flashlight_torch_high)) {
		...
		pr_err("%s : init err, flashlight_torch_high\n", __func__);
	}
	...
	if (IS_ERR(flashlight_torch_low)) {
		...
		pr_err("%s : init err, flashlight_torch_low\n", __func__);
	}
	...
	if (IS_ERR(flashlight_flash_high)) {
		...
		pr_err("%s : init err, flashlight_flash_high\n", __func__);
	}
	...
	if (IS_ERR(flashlight_flash_low)) {
		...
		pr_err("%s : init err, flashlight_flash_low\n", __func__);
	}
...
int flashlight_gpio_set(int pin , int state)
{
	...
	if (IS_ERR(flashlight_pinctrl)) {
		pr_err("%s : set err, flashlight_pinctrl not available\n", __func__);
		return -1;
	}
	...
	case FLASHLIGHT_PIN_HWEN:
		if (state == STATE_LOW && !IS_ERR(flashlight_hwen_low))
			...
			pr_err("%s : set err, pin(%d) state(%d)\n", __func__, pin, state);
		break;
		...
		for (j = 0; j < e_Max_Strobe_Num_Per_Dev; j++)
			for (k = 0; k < e_Max_Part_Num_Per_Dev; k++) {
				if (g_pFlashInitFunc[i][j][k] != 0) {
					pr_err("checkAndRelease %d %d %d", i, j, k);
...
int getSensorDevIndex(int sensorDev)
{
	if (sensorDev == e_CAMERA_MAIN_SENSOR)
		...
		return 2;
	pr_err("sensorDev=%d is wrong", sensorDev);
	return -1;
}
...
int getStrobeIndex(int strobeId)
{
	if (strobeId < 1 || strobeId > 2) {
		pr_err("strobeId=%d is wrong", strobeId);
		return -1;
	}
	return strobeId - 1;
}
...
int getPartIndex(int partId)
{
	if (partId < 1 || partId > 2) {
		pr_err("partId=%d is wrong", partId);
		return -1;
	}
	return partId - 1;
}
...
{
	pr_err("[default_flashlight_open] E ~");
	return 0;
}

MINT32 default_flashlight_release(void *pArg)
{
	pr_err("[default_flashlight_release] E ~");
	return 0;
}
...
MINT32 default_flashlight_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASHLIGHTIOC_G_FLASHTYPE:
		...
		if (copy_to_user((void __user *)arg, (void *)&kdArg, sizeof(kdStrobeDrvArg))) {
			pr_err("[FLASHLIGHTIOC_G_FLASHTYPE] ioctl copy to user failed ~");
			return -EFAULT;
		}
		break;
	default:
		pr_err("[default_flashlight_ioctl] ~");
		break;
	}
	return i4RetValue;
}
...
static int setFlashDrv(int sensorDev, int strobeId)
{
	...
	strobeIndex = getStrobeIndex(strobeId);
      pr_err("setFlashDrv sensorDev=%d, strobeId=%d", sensorDev, strobeId);
      pr_err("setFlashDrv sensorDevIndex=%d, strobeIndex=%d", sensorDevIndex, strobeIndex);
	...
	pr_err("setFlashDrv sensorDev=%d, strobeId=%d, partId=%d ~", sensorDev, strobeId, partId);
	...
	if ((*ppF) != 0) {
		...
		pr_err("setFlashDrv ok %d", __LINE__);
	} else {
		pr_err("set function pointer not found!!");
		return -1;
	}
	return 0;
}
...
static int decFlash(void)
{
	...
	for(k=0;k<e_Max_Part_Num_Per_Dev;k++)
	{
		if(g_pFlashInitFunc[i][j][k]!=0)
		{
			...
			pr_err("decFlash i,j,k,duty %d %d %d %d", i, j, k, duty);
...
static int closeFlash(void)
{
	...
	for (i = 0; i < e_Max_Sensor_Dev_Num; i++) {
		/* pr_err("closeFlash ln=%d %d",__LINE__,i); */
		for (j = 0; j < e_Max_Strobe_Num_Per_Dev; j++) {
			/* pr_err("closeFlash ln=%d %d",__LINE__,j); */
			for (k = 0; k < e_Max_Part_Num_Per_Dev; k++) {
				/* pr_err("closeFlash ln=%d %d %d",__LINE__,k, (int)g_pFlashInitFunc[i][j][k]); */
				if (g_pFlashInitFunc[i][j][k] != 0) {
...
{
	pr_err("Lbat_protection_powerlimit_flash %d (%d %d %d %d)\n", level, LOW_BATTERY_LEVEL_0,
...
{
	pr_err("bat_per_protection_powerlimit_flashlight %d (%d %d %d)\n", level,
...
static long flashlight_ioctl_core(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	copyRet = copy_from_user(&kdArg, (void *)arg, sizeof(kdStrobeDrvArg));
	pr_err("flashlight_ioctl cmd=0x%x(nr=%d), senorDev=0x%x ledId=0x%x arg=0x%lx", cmd,
	...
	case FLASH_IOC_IS_LOW_POWER:
		pr_err("FLASH_IOC_IS_LOW_POWER");
		{
			...
			|| gLowPowerVbat != LOW_BATTERY_LEVEL_0)
				isLow = 0;
			pr_err("FLASH_IOC_IS_LOW_POWER %d %d %d", gLowPowerPer, gLowPowerVbat, isLow);
			kdArg.arg = isLow;
			if (copy_to_user
			    ((void __user *)arg, (void *)&kdArg, sizeof(kdStrobeDrvArg))) {
				pr_err("[FLASH_IOC_IS_LOW_POWER] ioctl copy to user failed ~");
				return -EFAULT;
			}
		}
		break;
	...
	case FLASH_IOC_GET_MAIN2_PART_ID:
		...
		//	if (dual_flash_id == 0x2)
		//		g_strobePartId[0][1] = 2;
			//ZTEMT:wangkai add for dual flash comp---End
			g_strobePartId[sensorDevIndex][strobeIndex] = partId;
			kdArg.arg = partId;
			if (copy_to_user
			    ((void __user *)arg, (void *)&kdArg, sizeof(kdStrobeDrvArg))) {
				pr_err("[FLASH_IOC_GET_PART_ID] ioctl copy to user failed ~");
				return -EFAULT;
			}
			pr_err("FLASH_IOC_GET_PART_ID line=%d partId=%d", __LINE__, partId);
		}
		break;
	case FLASH_IOC_SET_ONOFF:
		{
			FLASHLIGHT_FUNCTION_STRUCT *pF;
       pr_err("FLASHLIGHT_ONOFF  111 : %d\n", kdArg.arg);
			...
			if (pF != 0) {
				...
				pr_err("[FLASH_IOC_SET_ONOFF] function pointer is wrong -");
			}
		}
		break;
	case FLASH_IOC_UNINIT:
		{
			...
			if (pF != 0) {
				...
				pr_err("[FLASH_IOC_UNINIT] function pointer is wrong ~");
			}
		}
		break;
	default:
		{
			...
			if (pF != 0)
				...
				pr_err("[default] function pointer is wrong ~");
		}
		break;
	}
	return i4RetValue;
...
static long my_ioctl_compat(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	pr_err("flash my_ioctl_compat2 line=%d cmd=%d arg=%ld\n", __LINE__, cmd, arg);
...
/* ======================================================================== */
#define ALLOC_DEVNO
//ZTETM: added by congshan start
static ssize_t show_flash_bkl(struct device *dev, struct device_attribute *attr, char *buf)
{

	return sprintf(buf, "%u\n", 1);
}

static ssize_t store_flash_bkl(struct device *dev, struct device_attribute *attr, const char *buf,
			 size_t size)
{
	
	return size;
}
static DEVICE_ATTR(flash_bkl, 0664, show_flash_bkl, store_flash_bkl);
//ZTETM: added by congshan end
static int flashlight_probe(struct platform_device *dev)
{
	int ret = 0, err = 0;

	pr_err("[flashlight_probe] start ~");

#ifdef ALLOC_DEVNO
	ret = alloc_chrdev_region(&flashlight_devno, 0, 1, FLASHLIGHT_DEVNAME);
	if (ret) {
		pr_err("[flashlight_probe] alloc_chrdev_region fail: %d ~", ret);
		goto flashlight_probe_error;
	} else {
		pr_err("[flashlight_probe] major: %d, minor: %d ~", MAJOR(flashlight_devno),
	...
	if (err) {
		pr_err("[flashlight_probe] cdev_add fail: %d ~", err);
		goto flashlight_probe_error;
	}
#else
#define FLASHLIGHT_MAJOR 242
	...
	if (ret != 0) {
		pr_err("[flashlight_probe] Unable to register chardev on major=%d (%d) ~",
	...
	//ZTETM: added by congshan start
	ret = device_create_file(&dev->dev, &dev_attr_flash_bkl);
	if (ret) {
    	logI("device_create_file flash_bkl fail!\n");
    }
	//ZTETM: added by congshan end

	pr_err("[flashlight_probe] Done ~");
	return 0;
...
static void flashlight_shutdown(struct platform_device *dev)
{
	...
	pr_err("[flashlight_shutdown] start\n");
	checkAndRelease();
	pr_err("[flashlight_shutdown] Done ~");
}
...
static int __init flashlight_init(void)
{
	...
	if (ret) {
		pr_err("[flashlight_probe] platform_device_register fail ~");
		return ret;
	}

	ret = platform_driver_register(&flashlight_platform_driver);
	if (ret) {
		pr_err("[flashlight_probe] platform_driver_register fail ~");
		return ret;
	}
