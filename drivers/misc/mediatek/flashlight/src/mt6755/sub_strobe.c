...
int FL_Enable_sub(void)
{
	...
	//PK_DBG("kangxiong FL_Enable_sub:gDuty = %d\n", gDuty);
	...
	if(gDuty ==1)  //pre-flash
	{
		...
		//LED2 torch 100ma
		if(deviceID ==0x02){
			KD2684_write_reg(0x6, 0x08);//(0x10<<1)|(readReg(0x6)&0x81)
		}else{
			KD2684_write_reg(0x6, 0x1);//(0x10<<1)|(readReg(0x6)&0x81)
		}
		buf[0] = 0x1;
		buf[1] = 0x7A;//0xAB;
	}
...
int FL_Init_sub(void)
{
	...
	PK_ERR(" FL_Init_sub line=%d deviceID = %d\n", __LINE__,deviceID);
	return 0;
}
...
static int sub_strobe_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	iowr_shift = cmd - (_IOWR(FLASHLIGHT_MAGIC, 0, int));
         ...
         //PK_DBG("kangxiong :sub dummy ioctl\n");	
	/*PK_DBG
	...
	case FLASH_IOC_SET_ONOFF:
		PK_DBG("FLASHLIGHT_ONOFF: %d  gDuty = %d bkl_mode = %d deviceID =%d\n", (int)arg,gDuty,bkl_mode,deviceID);	
		if (arg == 1) {
...
{
	PK_DBG(" sub_strobe_release release");
	if (strobe_Res) {
		spin_lock_irq(&g_strobeSMPLock);
		atomic_set(&camera_bklight_set, 0);
