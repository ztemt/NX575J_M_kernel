...
int keymaster_open(struct inode *inode, struct file *filp)
{
    ...
    printk("!!!!!microtrust kernel open keymaster dev operation!!!\n");

    if(keymaster_devp != NULL)
        filp->private_data = keymaster_devp;
    else {
        printk("microtrust keymaster_devp is NULL\n");
        return -EINVAL;
    }
    return 0;
}
...
static int keymaster_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
    case CMD_MEM_CLEAR:
        printk(KERN_INFO "microtrust keymaster mem clear. \n");
        break;
    case CMD_MEM_SEND:
        //printk("microturst ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
        //printk("microtrust kernel keymaster CMD_MEM_SEND\n");
        /*
        fp_cid = virt_to_phys(&fp_fid);
        printk("chx share mem: fp_fid=%d,fp_cid = %d, len=%d\n",fp_fid, fp_cid,sizeof(unsigned int));
        ...
        */
        if (!keymaster_buff_addr) {
            printk("microtrust keymaster fp_buiff_addr is invalid!. \n");
        ...
        if (copy_from_user((void *)keymaster_buff_addr, (void *)arg,
                           KEYMASTER_SIZE)) {
            printk(KERN_ERR "microturst keymaster copy from user failed. \n");
            up(&keymaster_api_lock);
            return -EFAULT;
        }
        //memcpy(&keymaster_msg, (void *)keymaster_buff_addr, sizeof(tciMessage_t));
        //printk("microtrust keymaster cmd is:id=%d,type=%d\n",keymaster_msg.command.commandId,keymaster_msg.rsa_gen_key.type);
        Flush_Dcache_By_Area((unsigned long)keymaster_buff_addr,
                             keymaster_buff_addr + KEYMASTER_SIZE);


        //this para 'KEYMASTER_DRIVER_ID' is not used
        send_keymaster_command(KEYMASTER_DRIVER_ID);

        if (copy_to_user((void *)arg, (unsigned long)keymaster_buff_addr,
                         KEYMASTER_SIZE)) {
            printk("microtrust keymaster copy from user failed. \n");
            up(&keymaster_api_lock);
            return -EFAULT;
        }

        //printk("microtrust keymaster ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~end\n");
        break;
    case CMD_NOTIFY_UTD:
    ...
    case CMD_FIRST_TIME_BOOT:
        //printk("teei daemon tell the keymaster this is first time boot start\n");
        ...
        //printk("teei daemon tell the keymaster this is first time boot end\n");
        break;
    default:
        up(&keymaster_api_lock);
        return -EINVAL;
...
static void keymaster_setup_cdev(struct keymaster_dev *dev, int index)
{
    ...
    if (err) {
        printk(KERN_NOTICE "Error %d adding keymaster %d.\n", err, index);
    }
}
...
void keymaster_init(void)
{
    ...
    if (IS_ERR(driver_class)) {
        result = -ENOMEM;
        printk("ut_keymaster class_create failed %d.\n", result);
    ...
    if (!class_dev) {
        result = -ENOMEM;
        printk("ut_keymaster class_device_create failed %d.\n", result);
        goto class_destroy;
    }
    keymaster_devp = NULL;
    ...
    printk("[%s][%d]create the ut_keymaster device node successfully!\n", __func__,
           __LINE__);
    goto return_fn;
