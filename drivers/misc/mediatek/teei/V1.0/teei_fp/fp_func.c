...
int fp_open(struct inode *inode, struct file *filp)
{
    if(wait_teei_config_flag == 1) {
        int ret;
        printk("[I]%s : Teei_config_flag = %lu\n", __func__, teei_config_flag);
        ret = wait_event_timeout(__fp_open_wq, (teei_config_flag == 1), msecs_to_jiffies(1000 * 10));
        if(ret == 0) {
            printk("[E]%s : Tees's loading is not finished, and has already waited 10s.\n", __func__);
            return -1;
        }
        if(ret < 0) {
            printk("[E]%s : Wait_event_timeout error.\n", __func__);
            return -1;
        }
        printk("[I]%s : Load tees finished, and wait for %u msecs\n", __func__, (1000 * 10 - jiffies_to_msecs(ret)));
...
#ifdef FP_DEBUG
        printk("case CMD_GATEKEEPER_CMD\n");
#endif
        if (!gatekeeper_buff_addr) {
            printk("gatekeeper_buff_addr is invalid!. \n");
            up(&fp_api_lock);
            return -EFAULT;
        }
#ifdef FP_DEBUG
        printk("varify gatekeeper_buff_addr  ok\n");
        printk("the value of gatekeeper_buff_addr is %lu\n", gatekeeper_buff_addr);
#endif
...
#ifdef FP_DEBUG
        printk("Flush_Dcache_By_Area  ok\n");
#endif
        send_gatekeeper_command(GK_DRIVER_ID);
#ifdef FP_DEBUG
        printk("send_gatekeeper_command  ok\n");
#endif
