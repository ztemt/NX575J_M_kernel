...
//ZTEMT:tanyijun add for set avdd above 2.8V start
/* Regulator vcama set_voltage_cal */
static int pmic_ldo_vcama_set_voltage_cal(struct regulator_dev *rdev, int uV)
{
	const struct regulator_desc *rdesc = rdev->desc;
	struct mtk_regulator *mreg;
	...
	val_offset = uV -2800000;
       if(val_offset > 0)
       	{
	    regval = 0xf -val_offset/20000;

	    if(regval <= 0x8)
	        regval = 0x8;
       	}else if(val_offset <= 0)
       	{
            regval = val_offset / (-20000);
       	    if( 0x7 <= regval)
	        regval = 0x7;
	}
	pr_err("tanavddldo pmic_ldo_vcama_set_voltage_cal uV = %d regval: %d\n",uV, regval);
	
	mreg = container_of(rdesc, struct mtk_regulator, desc);
	
	mt6353_upmu_set_rg_vcama_cal(regval);
...
static int pmic_ldo_vcama_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
	...
	mreg->vosel.cur_sel = selector;
	PMICLOG("tanavddldo vcama set_voltage_sel: %d\n", selector);
	/*return mtk_regulator_set_voltage_sel(rdev, selector);*/
...
static struct regulator_ops pmic_ldo_vcama_ops = {
	...
	.set_voltage_cal = pmic_ldo_vcama_set_voltage_cal, //tanyijun add  for set avdd to 2.8 ~2.96V
