 ...
 ******************************************************************************/
#if defined(CONFIG_MTK_BQ25896_SUPPORT) || defined(CONFIG_TI_BQ25890_CHARGER_SUPPORT)
...
#if defined(CONFIG_NUBIA_GENERAL_CHARGE_COMMON)
#include <linux/power_supply.h>
extern bool mtk_is_host_mode(void);

static int set_chrsts_int_task(void)
{
...
void chrdet_int_handler(void)
{
	...
	pmic_crit("6353[chrdet_int_handler]CHRDET status = %d....\n", pmic_get_register_value(PMIC_RGS_CHRDET));
	...
	if (!upmu_get_rgs_chrdet()) {
		...
		#if defined(CONFIG_MTK_BQ25896_SUPPORT) || defined(CONFIG_TI_BQ25890_CHARGER_SUPPORT)
			if(pmic_get_register_value(PMIC_RGS_VCDT_HV_DET)) bq25890_set_en_hiz(0x1);
		#endif
		#if defined(CONFIG_NUBIA_PM_COMMON_FEATURE)
			/** Not Poweroff device in the kernel mode.*/
		#else
			mt_power_off();
		#endif
		}
