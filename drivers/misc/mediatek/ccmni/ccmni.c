...
static int ccmni_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCCCMNICFG:
		...
		if (ccmni->index >= ctlb_irat->ccci_ops->ccmni_num) {
			CCMNI_ERR_MSG(md_id, "SIOCSCCMNICFG: %s iRAT(MD%d->MD%d) fail, ccmni_idx(%d) > md%d_ccmni_num(%d)\n",
				dev->name, md_id, md_id_irat, ccmni->index, md_id_irat, ctlb_irat->ccci_ops->ccmni_num);
			break;
		}
