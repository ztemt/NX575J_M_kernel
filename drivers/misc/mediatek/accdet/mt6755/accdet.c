...
#ifdef CONFIG_ACCDET_HPHR_IRQ
static int irq_number=0;
static void hphr_irq_fn(struct work_struct *unused);
static DECLARE_DELAYED_WORK(hphr_work,hphr_irq_fn);
#endif
...
#ifdef CONFIG_ACCDET_HPHR_IRQ
void schedule_hphr_irq(void)
{
  irq_number++;
  ACCDET_DEBUG("[ACCDET]%s::-------------irq_number====%d\n",__func__,irq_number);
  if(schedule_delayed_work(&hphr_work,msecs_to_jiffies(1000))==0) {
  	  ACCDET_DEBUG("[ACCDET]%s::schedule_delayed_work-------\n",__func__);
  	}
}
#endif
...
#ifdef CONFIG_ACCDET_HPHR_IRQ    //correct headset to headphone
static void hphr_irq_fn(struct work_struct *unused)
{
//   int current_status=0;
 ...
 //   current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0) >> 6);
   ...
   pin_adc_value = Accdet_PMIC_IMM_GetOneChannelValue(1);
	ACCDET_DEBUG("[Accdet]%s:::::::::::::before adc read, irq_number=%d\n", __func__,irq_number);
if(pin_adc_value<50)
   {	accdet_status = PLUG_OUT;
	cable_type = NO_DEVICE;
    pr_debug("%s: wrt:::accdet_irq_handler\n",__func__);
   	accdet_workqueue_func();
}
}
#endif
