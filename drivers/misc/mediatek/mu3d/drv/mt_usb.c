...
extern bool is_usb_cable_connect(void);
...
void connection_work(struct work_struct *data)
{
	...
	if (!usb_phy_check_in_uart_mode()) {
		...
		os_printk(K_CRIT, "%s type=%d, cmode_effect_on=%d, usb_mode:%d\n",
		...
		if (!mt_usb_is_device()) {
			...
			musb->is_clk_on = 0;
			os_printk(K_CRIT, "%s, Host mode. directly return\n", __func__);
			return;
		}
#endif

		os_printk(K_CRIT, "%s musb %s, cable %s\n", __func__,
...
void mt_usb_connect(void)
{
	...
	if (_mu3d_musb) {
		...
		os_printk(K_CRIT, "%s musb_musb not ready\n", __func__);
	}
	os_printk(K_INFO, "%s-\n", __func__);
}
...
extern int chrdet_connect_state(void);
bool usb_cable_connected(void)
{
#ifndef CONFIG_MTK_FPGA

#ifndef CONFIG_NUBIA_GENERAL_CHARGE_COMMON
	...
	if (upmu_get_rgs_chrdet() && (chg_type == STANDARD_HOST))
       ...
       os_printk(K_INFO, "%s chrdet_connect_state=%d\n", __func__, chrdet_connect_state());
	if( chrdet_connect_state())
		return true;
#else
